...
MAX_HANDS = 12
HIRE_WINDOW = 9
JOBS_PER_UNIT = 8.0      # conservative allowance for walking between tile ops
HOLD_EARLY = 0.62
HOLD_LATE = 0.12
RELAX_DAY = 18
...
# Sellable units a worker may carry before it is force-marched to the shed.
#
# Was hardcoded 4.  That is a *forced* action -- it overrides the assignment
# solver outright -- and a single harvest hands a unit several units at once, so
# workers were constantly abandoning adjacent work to run to the centre.  We
# DROP ~199 times an episode; top players (91566965, 91569740) DROP 58 and still
# sell more.
#
# Carrying is free: the engine's _inv_add has no capacity cap
# (kaggriculture.py:286), and at every dawn _drop_inventories_to_shed banks every
# worker's whole inventory into the shed at no action cost (:830-844, :865).
# So most mid-day shed trips buy nothing the nightly deposit would not.
#
# Bounded, not removed: the shed caps at 100 and dawn overflow is DISCARDED
# (:841-843), so a worker hoarding without limit would throw goods away. Sales
# also draw only from the shed (:640-642), so stock has to land there to be sold.
BANK_LOAD = 12
...
def _plan(obs):
    ...
    for idx, pos in units:
        ...
        must_bank = (step >= DUMP_STEP or sale_load >= BANK_LOAD
                     or money < 300.0)
