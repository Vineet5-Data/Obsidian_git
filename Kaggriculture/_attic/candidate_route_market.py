...
COMPETITOR_SUPPLY_MULT = 2
SHED_PRESSURE = 82
FORCED_LIQUIDATION_STEP = 696
...
def _price_at(item, inventory):
    base, throughput, below_fn, below_target, above_fn, above_target = MARKET_PARAMS[item]
    delta = int(inventory) - I0
    if delta < 0:
        amplitude = below_target * base / _curve(below_fn, throughput)
        return max(1, int(round(base + amplitude * _curve(below_fn, -delta))))
    amplitude = above_target * base / _curve(above_fn, throughput)
    return max(1, int(round(base - amplitude * _curve(above_fn, delta))))


def _sellable_units(item, inventory, have, minimum):
    lo, hi = 0, max(0, int(have))
    multiplier = max(1, int(COMPETITOR_SUPPLY_MULT))
    while lo < hi:
        middle = (lo + hi + 1) // 2
        if _price_at(item, int(inventory) + multiplier * middle - 1) >= minimum:
            lo = middle
        else:
            hi = middle - 1
    return lo
...
def _projected_shed(obs, action):
    ...
    for index, unit_action in enumerate(unit_actions):
        if index >= len(positions) or index >= len(inventories) or not unit_action:
            continue
        try:
        ...
        if unit_action[0] == "DROP":
            ...
            try:
                ...
                continue
            deposits = ((item, min(max(0, requested), int(inventory.get(item, 0) or 0))),)
        else:
            continue
...
def _adaptive_market(obs, action, step):
    action = _aligned(action, obs)
    available = _projected_shed(obs, action)
    market = _get(obs, "market", {}) or {}
    inventory = dict(_get(market, "inventory", {}) or {})
    pressure = sum(available.values()) >= int(SHED_PRESSURE)
    forced = int(step) >= int(FORCED_LIQUIDATION_STEP)
    result = []
    sold = set()

    def sale(item, requested=None):
        if item in sold or item not in MARKET_PARAMS:
            return None
        have = max(0, int(available.get(item, 0) or 0))
        if requested is not None:
            have = min(have, max(0, int(requested)))
        if have <= 0:
            return None
        floor = 1 if forced or pressure else int(MIN_SELL_PRICE[item])
        quantity = _sellable_units(item, int(inventory.get(item, I0) or I0), have, floor)
        if quantity <= 0:
            return None
        sold.add(item)
    ...
    for raw in action.get("market", []) or []:
        ...
        if order and order[0] == "SELL" and len(order) >= 3:
            ...
            quantity = min(requested, max(0, int(available.get(item, 0) or 0)))
            if quantity > 0:
                result.append(["SELL", item, quantity])
                sold.add(item)
                available[item] = max(0, int(available.get(item, 0) or 0) - quantity)
        else:
            result.append(order)

    # Capture transient scarcity between the recorded route's batch-sale turns.
    candidates = []
    prices = dict(_get(market, "prices", {}) or {})
    for item in ("STRAWBERRY", "MELON", "MILK", "WOOL", "CARROT", "TOMATO", "EGG"):
        if item in sold or int(available.get(item, 0) or 0) <= 0:
            continue
        floor = 1 if forced or pressure else int(MIN_SELL_PRICE[item])
        quantity = _sellable_units(
            item, int(inventory.get(item, I0) or I0), int(available[item]), floor
        )
        if quantity > 0:
            candidates.append((int(prices.get(item, 1) or 1) * quantity, item, quantity))
    candidates.sort(reverse=True)
    for _value, item, quantity in candidates:
        if len(result) >= 10:
            break
        result.append(["SELL", item, quantity])
...
def agent(obs):
    try:
        ...
        return _adaptive_market(obs, action, step)
    except Exception:
        farms = list(_get(obs, "farms", []) or [])
        seat = 1 if int(_get(obs, "player", 0) or 0) == 1 else 0
        farm = farms[seat] if seat < len(farms) else {}
