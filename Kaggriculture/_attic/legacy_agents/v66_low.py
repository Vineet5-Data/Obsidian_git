...
def _plan(obs):
    ...
    for x, y, t in beasts:
        ...
        # Softer form of the v66_alt idea.  Production does not require feeding
        # (engine :808-814 adds the base unit regardless of `fed_today`), so a
        # non-starving animal is worth topping up only if a worker has nothing
        # better to do.  Dropping 2.2 -> 0.25 demotes those FEED jobs to filler
        # instead of removing them, which keeps a day of slack before the
        # 2-consecutive-unfed escape rather than betting on never missing the
        # one mandatory day.
        if step < FINAL_FARM_STEP and not t.get("fed_today"):
            unfed += 1
            starving = int(t.get("consecutive_unfed", 0) or 0) >= 1
            jobs.append(((14.0 if starving else 0.25) * p,
                         (x, y), ["FEED"], "WHEAT"))
