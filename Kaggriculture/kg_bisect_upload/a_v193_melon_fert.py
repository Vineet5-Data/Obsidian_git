...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    fert_due = False
    if step < FINAL_FARM_STEP and not at_cap and not doomed and int(tile.get("fertilized_until_day", -1) or -1) < day:
        planted_day = tile.get("planted_day", day)
        planted_day = day if planted_day is None else int(planted_day)
        if cd["ong"]:
            ...
            if due and (product_price - fertilizer_spot > 12.0 or strategic_denial):
                fert_due = True
        elif crop_name == "MELON" and held < cd["my"]:
            window_start = (cd["myd"] + 1) // 2
            if window_start <= age < cd["myd"] and product_price - fertilizer_spot > 20.0:
                fert_due = True
    if fert_due:
            fertilizer_jobs = 1
            denial_bonus = 0.0
