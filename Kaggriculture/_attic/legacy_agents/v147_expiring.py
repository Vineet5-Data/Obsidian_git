...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if held > 0 and age >= cd["fyd"]:
        ...
        if (product_price < 50
            ...
            # Both branches are stock at risk: a capped tile discards its
            # daily tick, and an expiring one is being destroyed a unit every
            # two steps by `_decay_plants` before the tile becomes a WEED.
            # Gating on `not expiring` sent the urgent case to the *lower*
            # spot price, ranking the safe tile above the doomed one.
            if at_cap or expiring:
                harvest_price = max(
                    product_price,
