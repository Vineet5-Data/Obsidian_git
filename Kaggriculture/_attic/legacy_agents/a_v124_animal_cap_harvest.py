"""v124 -- v123b plus the same at-risk harvest rule for animals.
...
days 0-19 phase keeps v122 behavior.

v124 stacks the identical correction onto animal_service_jobs, the gate's
second call site.  There MELON and STRAWBERRY are the dead entries and the
gate only ever zeroes MILK and WOOL; near_cap's 2.0x multiplier cannot rescue
it because 0.0 * 2.0 is 0.0.  Animals never expire and have no lifetime yield
cap, so one sitting at max_held discards a production every interval for the
rest of the game and burns its accumulated CARE bonus too -- an unbounded
leak, where a crop's is capped by its lifespan.  Telemetry: animal cap ticks
rise from MILK 1.99 / WOOL 1.63 per day in days 0-14 to MILK 11.11 /
WOOL 12.08 in days 25-29.  Only near_cap animals are rescued, mirroring
v123b.  animal_service_jobs gains a storage_load parameter so it can reuse
hold_fraction; day is derived as step // TPD, exactly as _plan computes it.  The floor is the agent's own selling reservation, so it is
...
def animal_service_jobs(x, y, tile, step, storage_load, spot):
    ...
    if held > 0:
        ...
        # Same defect as the crop path, second call site.  MELON and
        # STRAWBERRY are dead entries here, so this gate only ever zeroes
        # MILK and WOOL -- and the near_cap doubling cannot rescue it,
        # because 0.0 * 2.0 is still 0.0.
        #
        # Animals are worse off than crops: they never expire and have no
        # lifetime yield cap, so one sitting at max_held discards a
        # production every interval for the rest of the game and burns its
        # accumulated CARE bonus with it.  The leak is unbounded, where a
        # crop's is capped by its lifespan.  Only rescue an animal that is
        # actually at risk of discarding, matching v123b's rule.
        if (product_price < 50
                and animal["prod"] in ("MELON", "STRAWBERRY", "MILK", "WOOL")):
            harvest_price = (
                max(product_price,
                    hold_fraction(step, step // TPD, storage_load)
                    * MP[animal["prod"]]["base"])
                if near_cap else 0.0)
        else:
...
def _plan(obs):
    ...
    for x, y, t in beasts:
        tile_jobs, tile_unfed = animal_service_jobs(
            x, y, t, step, shed_used + sum(carried_all.values()), spot)
