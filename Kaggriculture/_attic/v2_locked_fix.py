...
Design comes from measured data (V2_SPEC.md), not intuition:
  * per-tile season value: COW 39 milk > SHEEP 38 wool > MELON > STRAWBERRY(fert) 8u > GOOSE
  * every premium market is hard-capped by town drain, so the tile mix saturates each
    drain exactly: 11 cow + 9 sheep + 12 melon + ~56 strawberry + wheat for feed
  * EGG/WHEAT have log glut curves (uncapped); everything else floors fast, so never
    push market inventory above I0
  * CARE triples animal output; fertilizer doubles strawberry output
  * v1 lost its money to (a) buying wheat for an oversized herd, (b) 64.6% of actions
    spent moving -> small herd, and every pending job on a tile is done before moving on
"""
    ...
    for i, pos in enumerate(w.units):
        if pos in w.shed_tiles:
            if load[i] and (w.turns_left_today <= 3 or load[i] >= 16):
                acts[i] = ["DROP"]
                continue
            need = min(6, unfed) - inv[i].get("WHEAT", 0)
...
def market(w):
    ...
    floor = 0 if endgame else 40 + 25 * herd
    ...
    if not endgame and short > 0 and herd > 0 and wprice <= WHEAT_BUY_MAX and shed_room > 2:
        k = min(short, max(0, int((money - 20) // max(1, wprice))), BUY_CHUNK)
        if k > 0:
    ...
    # wheat acreage is life support for the herd: cheap, and it comes before luxuries
    ...
    if w.day <= 18 and shed_room > 6:
        if n_cow < TGT_COW:
            k = min(TGT_COW - n_cow, int(max(0, money - floor) // 400), 2)
            if k > 0:
                buy(400 * k, ["BUY_ANIMAL", "COW", k])
        if n_sheep < TGT_SHEEP and n_sheep <= n_cow:
            k = min(TGT_SHEEP - n_sheep, int(max(0, money - floor) // 500), 2)
            if k > 0:
                buy(500 * k, ["BUY_ANIMAL", "SHEEP", k])

    if slots > 0:
        have = w.counts.get("MELON", 0) + w.seeds.get("MELON", 0)
        if w.day <= LAST_MELON_DAY and have < TGT_MELON:
        ...
        if 7 <= w.day <= LAST_STRAW_DAY and have < TGT_STRAW and slots > 0:
    ...
    nq = len(w.quads) - 1
    if nq < MAX_EXTRA_QUADS and 3 <= w.day <= LAND_LAST_DAY:
        cost = (1000, 2000, 4000)[nq]
        if money - cost >= floor + (300 if len(w.empty) < 6 else 1500):
            buy(cost, ["BUY_LAND"])
