...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if held > 0 and age >= cd["fyd"]:
        ...
        # v160 Context-Aware Grow-First Logic
        season_ending = step >= (30 * 24 - 24) # Harvest whatever we have on the final day (Day 29)
        still_growing = (not cd["ong"] and age < cd["myd"] and held < cd["my"]
                         and not expiring and not season_ending)
        
        # Block early harvest only in the mid/late game (Day 15+). 
        # In Days 0-14, fast Wheat (even at 1 unit) is critical to buy Geese immediately.
        if still_growing and day >= 15:
            pass # Block premature harvest
        else:
