...
AGENT_PATH = "a_v144_pure_unleashed.py"
REPLAYS = [
...
for name, path in REPLAYS:
    print(f"\n=======================================================")
    print(f"Testing a_v144_pure_unleashed against {name}")
    print(f"=======================================================")
    ...
    # Match 1: Opponent Tape (Seat 0) vs v144 (Seat 1)
    ...
    w1 = "v144 WON!" if s1 > s0 else "Opponent Tape WON"
    print(f"--- Match 1: Opponent (Seat 0) vs v144 (Seat 1) ---")
    print(f"  Opponent: {s0:.0f} | v144: {s1:.0f} -> {w1} (Margin: {s1 - s0:+.0f})")
    if s1 > s0: total_v144_wins += 1
    total_matches += 1

    # Match 2: v144 (Seat 0) vs Opponent Tape (Seat 1)
    ...
    w2 = "v144 WON!" if s0 > s1 else "Opponent Tape WON"
    print(f"--- Match 2: v144 (Seat 0) vs Opponent (Seat 1) ---")
    print(f"  v144: {s0:.0f} | Opponent: {s1:.0f} -> {w2} (Margin: {s0 - s1:+.0f})")
    if s0 > s1: total_v144_wins += 1
    total_matches += 1

print("\n=======================================================")
print(f"FINAL REPLAY TOURNAMENT TALLY: v144 Won {total_v144_wins} / {total_matches} matches ({total_v144_wins/total_matches*100:.1f}%)")
print("=======================================================")
