...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    expiring = 0 <= lifespan <= step + 3 * TPD
    if held > 0 and age >= cd["fyd"]:
        ...
        gain = base_gain
        if cd["ong"] and held >= 2:
            gain = max(gain, 120.0 + held * float(product_price))
        # If the plant is expiring or doomed, rescue the held yield before decay destroys it.
        if expiring or doomed:
            gain += 2500.0 + held * max(80.0, float(product_price))
        if storage_load > 90:
            gain *= 0.1
...
def _plan(obs):
    ...
    cash = money
    n_beasts = len(beasts)
    days_to_end = max(0, 29 - day)
    feed_target = min(n_beasts + int(math.ceil(n_beasts * FEED_BUFFER)),
                      days_to_end * n_beasts) if step < FINAL_FARM_STEP else 0
    sim = dict(minv)
