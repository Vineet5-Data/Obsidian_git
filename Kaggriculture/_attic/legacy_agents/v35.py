"""Route A 90729118 + route B 90635979_p1, opponent-selected at step 200 (v35, retuned for engine 1.32.6)."""
...
SMOOTH_START = 250
SMOOTH_CAP = 5
# WINDOW re-tuned on engine 1.32.6: the town centre now fires every 24 steps
# (was 12) with a flat multiplier (was 1x/2x/4x by day), so the drain cycle that
# makes spreading profitable is twice as long and the advance window must match.
# Measured on the real current field: window 16 -> 79.2%, window 8 -> 75.0%,
# smoothing off -> 75.0%.  The old value 8 was fitted to the old clock.
SMOOTH_WINDOW = 16
