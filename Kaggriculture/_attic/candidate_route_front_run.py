...
FRONT_RUN_LOOKAHEAD = 1


def _front_run_sales(obs, action, step):
    ...
    current = [list(order) for order in (action.get("market", []) or [])]
    current_sales = set()
    for order in current:
        if len(order) >= 3 and order[0] == "SELL":
            current_sales.add(order[1])
            available[order[1]] = max(
                0, int(available.get(order[1], 0) or 0) - max(0, int(order[2] or 0))
            )
    future_step = int(step) + int(FRONT_RUN_LOOKAHEAD)
    if future_step >= len(_ACTIONS):
        return action
    prices = _get(_get(obs, "market", {}) or {}, "prices", {}) or {}
    candidates = []
    for order in (_ACTIONS[future_step].get("market", []) or []):
        if len(order) < 3 or order[0] != "SELL" or order[1] in current_sales:
            continue
        item = order[1]
        quantity = min(
            max(0, int(order[2] or 0)), max(0, int(available.get(item, 0) or 0))
        )
        if quantity:
            candidates.append((max(1, int(prices.get(item, 1) or 1)) * quantity, item, quantity))
    candidates.sort(reverse=True)
    early = [["SELL", item, quantity] for _value, item, quantity in candidates]
    if early:
        action["market"] = (early + current)[:10]
    return action
...
def agent(obs):
    try:
        ...
        return _front_run_sales(obs, _ACTIONS[step], step)
    except Exception:
        farms = list(_get(obs, "farms", []) or [])
        seat = 1 if int(_get(obs, "player", 0) or 0) == 1 else 0
        farm = farms[seat] if seat < len(farms) else {}
