...
For observable agents, telemetry is captured inside each worker and included in
the output JSON. Service-stage rates are printed for days 0-14, 15-19, 20-24,
and 25-29.
"""
import glob
import json
...
def telemetry_window_totals(rows, lo_day, hi_day):
    """Aggregate v117's per-day counters over one reporting window."""
    stages = ("created", "admitted", "assigned", "emitted",
...
def report_telemetry(cut_name, rows):
    """Print service-stage congestion and completion rates for observable agents."""
    if not any(r.get("telemetry") for r in rows):
        return
    print(f"\n-- SERVICE TELEMETRY: {cut_name} --")
    for label, lo_day, hi_day in TELEMETRY_WINDOWS:
        games, totals, caps, expiries, movement, passes = (
            telemetry_window_totals(rows, lo_day, hi_day))
        if not games:
            continue
        ...
        if caps:
            cap_text = ", ".join(
                f"{item}={count / game_days:.2f}/day"
                for item, count in sorted(caps.items()))
            print(f"    animal cap ticks: {cap_text}")
        if expiries:
            expiry_text = ", ".join(
                f"{item}={count / games:.2f}/game"
                for item, count in sorted(expiries.items()))
