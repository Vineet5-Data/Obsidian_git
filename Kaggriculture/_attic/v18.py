...
# The town drains a fixed number of units per day (roughly 18 strawberry and
...
def agent(obs):
    try:
        ...
        if step == 0:
            _RUNTIME["raw_opponent"] = False
        action = _quota_market(obs, _fill_idle_units(obs, _adapt_animals(obs, _ACTIONS[step])))
