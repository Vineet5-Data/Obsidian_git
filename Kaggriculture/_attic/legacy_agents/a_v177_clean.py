...
def animal_allocation_limits(day, days_left, quadrant_count, slots,
    ...
    if days_left < 8:
        ...
        target = min(30, max(6, int(round(25 * quadrant_count * 0.33))))
...
def animal_service_jobs(x, y, tile, step, spot, day, storage_load):
    ...
    if held > 0:
        ...
        if product_price < 50 and animal["prod"] in ("MILK", "WOOL"):
            if near_cap:
                harvest_price = max(product_price, hold_fraction(step, day, storage_load) * MP[animal["prod"]]["base"])
            else:
                harvest_price = 0.0
        else:
...
def _plan(obs):
    ...
    # ---- job board --------------------------------------------------------
    jobs = []
    storage_load = shed_used + sum(carried_all.values())
    unfed = 0
    fertilize_needed = 0
    for x, y, t in beasts:
        tile_jobs, tile_unfed = animal_service_jobs(x, y, t, step, spot, day, storage_load)
        jobs.extend(tile_jobs)
        unfed += tile_unfed

    for x, y, t in plants:
        tile_jobs, tile_fertilize = crop_service_jobs(
            x, y, t, step, day, storage_load, spot,
