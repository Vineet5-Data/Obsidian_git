...
def _preferred_animal(obs, counts):
    prices = _get(_get(obs, "market", {}) or {}, "prices", {}) or {}
    cow_value = COW_BIAS * ANIMAL_YIELD["COW"] * max(1, int(prices.get("MILK", 160) or 160)) / ANIMAL_COST["COW"]
...
def agent(obs):
    try:
        ...
        if step == 0:
            _RUNTIME["raw_opponent"] = False
        return _terminal_liquidation(obs, _adapt_animals(obs, _ACTIONS[step]))
    except Exception:
        farms = list(_get(obs, "farms", []) or [])
        seat = 1 if int(_get(obs, "player", 0) or 0) == 1 else 0
        farm = farms[seat] if seat < len(farms) else {}
