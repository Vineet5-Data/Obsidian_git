...
MAX_BUY_ORDERS = 5
WHEAT_BUY_MAX = 45
LOW_T_CAP = 2            # per-species cap for MILK/WOOL (vault #58)
...
def _plan(obs):
    ...
    def species_limit(name):
        """Vault #58: cap the species whose product market is thinnest.

        The flat 0.45 fraction allows 8 COWs and 8 SHEEP at a 16-animal target,
        but MILK and WOOL have the two lowest absorption thresholds in the game
        (`T` 122 and 105, against EGG's 332 and CARROT's 450), and both fall off
        hard once past it -- WOOL on a square curve (`af="sq"`, `at=3.20`).  A
        big dairy/wool herd therefore floods a market that cannot drain, and the
        extra animals are sold into a price our own earlier units destroyed.
        Geese are left alone: EGG absorbs nearly 3x as much.
        """
        return LOW_T_CAP if ANIMALS[name]["prod"] in ("MILK", "WOOL") else species_cap
    ...
    for name in sorted(ANIMALS, key=lambda n: -opp_animal_counts.get(n, 0)):
        ...
        owned = (own_animal_counts.get(name, 0)
                 ...
                 + int(carried_all.get(name, 0) or 0))
        strategic_target = min(species_limit(name), opp_animal_counts.get(name, 0))
