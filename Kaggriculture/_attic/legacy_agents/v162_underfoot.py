...
UNDERFOOT_EXTRA = 32     # cap on zero-travel jobs promoted into the matrix on
                         # top of the value-ranked window; bounds the O(n^3)
                         # Kuhn-Munkres cost against the 1s actTimeout
FEED_DAYS = 1
FEED_BUFFER = 0.80
MAX_BUY_ORDERS = 5
WHEAT_BUY_MAX = 45
...
def _plan(obs):
    ...
    # The board is ranked by raw `value`, but the matrix below optimises
    # `value - move_rate * d`.  A job on a worker's own tile has d == 0 -- the
    # objective's best case -- and was still being cut here whenever its raw
    # number missed the top-48 cutoff.  Measured on one game: 2,003 zero-travel
    # jobs discarded, 57.7% of under-foot CARE and 46.9% of under-foot
    # COLLECT_FERTILIZER, the two operations where the real leaderboard replays
    # show us walking 8.7x and 2.8x further per job than the top players.
    #
    # Admit them on top of the value-ranked window.  Extra columns cannot lower
    # a Kuhn-Munkres optimum, so this only widens what the existing pricing is
    # allowed to choose from; move_rate is still calibrated off the value-sorted
    # prefix below and is deliberately left alone.
    underfoot_pos = {p for _i, p in free_units}
    extra = [j for j in range(ncand, len(jobs))
             if jobs[j][1] in underfoot_pos][:UNDERFOOT_EXTRA]
    cands = jobs[:ncand] + [jobs[j] for j in extra]
    ncand = len(cands)
    telemetry_admitted_jobs = tuple(cands)
    BIG = 1e9
