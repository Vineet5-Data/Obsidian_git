...
def adjusted_sell_hold(base_hold, cash, projected_used, n_beasts,
    ...
    if projected_used > 40:
        sell_hold = min(sell_hold, 0.28)
    if projected_used >= 60:
        sell_hold = min(sell_hold, 0.15)
    ...
    for item, raw_qty in projected.items():
        ...
        if sell_hold <= 0.0:
            ...
            if item == "STRAWBERRY" and rival_qty > 0:
                reservation = 1.0
            # Retain fertilizer for ongoing Strawberry crops instead of market dumping
...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if (step < FINAL_FARM_STEP and cd["ong"]
        ...
        if due and (incremental > 0.0 or strategic_denial or crop_name == "STRAWBERRY"):
            fertilizer_jobs = 1
            denial_bonus = 0.0
            ...
            jobs.append((max(120.0, 2.0 * product_price - fertilizer_spot
                             + denial_bonus),
