...
def _plan(obs):
    ...
    if days_left < 8:
        ...
        visible_rival_herd = sum(opp_animal_counts.values())
        if visible_rival_herd > 0:
            animal_target = min(animal_target,
                                min(20, max(6, visible_rival_herd + 2)))
        ...
        for j, (value, tgt, op, req) in enumerate(jobs):
            ...
            if op[0] == "PICKUP":
                ...
                if int(inv.get(op[1], 0) or 0) >= cap:
                    continue
            if req in ("WHEAT", "FERTILIZER") and inv.get(req, 0) <= 0:
                continue
            d = dist(pos, tgt)
