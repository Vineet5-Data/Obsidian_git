...
def _plan(obs):
    ...
    empties.sort(key=lambda p: (min(dist(p, s) for s in SHED_SET), p[1], p[0]))
    ...
    animal_need = min(slots, max(0, animal_target - current_animal_assets))
    want_animal = {}
    species_cap = max(2, int(math.ceil(max(1, animal_target) * 0.45)))
    for _ in range(animal_need):
        best = None
        for name, a in ANIMALS.items():
            ...
            crowd = (already + 0.80 * opp_animal_counts.get(name, 0))
    ...
    crop_slots = max(0, min(60, slots - sum(want_animal.values())))
    want_crop = {}
    ...
    for _ in range(remaining_crop_slots):
        ...
        for crop, cd in CROPS.items():
            ...
            if gain <= 0:
                continue
            crowd = already + 0.80 * opp_crop_counts.get(crop, 0)
    ...
    for x, y, t in plants:
        ...
        if held > 0 and age >= cd["fyd"]:
            terminal = 5.0 if step >= DUMP_STEP else 1.0
            jobs.append(((held * p + (14 if not cd["ong"] else 0)) * terminal,
                         (x, y), ["HARVEST"], None))
    ...
    pickup_workers = min(len(shed_tiles()),
    ...
    if pickup_workers > 0 and shed.get("WHEAT", 0) > 0:
        ...
        for s in shed_tiles()[:pickup_workers]:
    ...
    for idx, pos in units:
        ...
        near = min(SHED_SET, key=lambda s: dist(pos, s))
        total_load = sum(max(0, int(v or 0)) for v in inv.values())
        if inv and pos in SHED_SET and bank_room >= total_load:
            acts[idx] = ["DROP"]
            bank_room -= total_load
        elif inv and pos in SHED_SET and bank_room > 0:
    ...
    for idx in sorted(acts):
        ...
        if not op or pos not in SHED_SET:
            continue
        if op[0] == "DROP":
    ...
    # never be crowded out by a long string of cheap hires.
    for _value, item, qty in rows:
        if len(orders) >= 10:
            break
        if sell_hold <= 0.0:
            n = qty
        else:
            floor = max(1.0, sell_hold * MP[item]["base"])
            n = min(qty, sell_cap(item, sim.get(item, I0), floor))
        if n <= 0:
            continue
