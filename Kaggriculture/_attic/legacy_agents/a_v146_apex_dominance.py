...
def crop_season(cd, days_left):
    ...
    # For ongoing crops that are fertilized, yields are doubled in practice.
    # Strawberry gets 2.0x to reflect full 8-unit fertilized yield.
    yield_multiplier = (2.0 if cd.get("seed") == 100 else 1.2) if cd["ong"] else 1.0
    ...
    for _ in range(remaining_crop_slots):
        ...
        for crop in crop_candidates:
            ...
            if gain <= 0:
                continue
            crowd = already
            time_risk = 1.0 + 0.015 * cd["fyd"]
            score = gain / (max(1.0, y[1]) * time_risk * (1.0 + 0.020 * crowd))
            if best is None or score > best[0]:
                best = (score, crop, y[0])
        if best is None:
            break
