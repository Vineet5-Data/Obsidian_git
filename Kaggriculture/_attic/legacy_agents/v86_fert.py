    ...
    for x, y, t in plants:
        ...
        if (step < FINAL_FARM_STEP
            ...
            if cd["ong"]:
                ...
                # Non-ongoing crops take the same x2 bonus -- the engine applies
                # it to every crop (kaggriculture.py:780-784), inside the
                # accumulation window
                #     (max_yield_day + 1) // 2 <= age_days <= max_yield_day
                # The old cd["ong"] gate skipped them entirely, so MELON (base
                # 250, my 6) was never once fertilized.  In the Kaggle loss
                # replays we applied 445 fertilizer to opponents' 760 while
                # *selling* 2,004 units at ~53 -- a unit applied to a watered
                # tile is worth ~3 extra yield, which beats the sale on every
                # crop we grow.
