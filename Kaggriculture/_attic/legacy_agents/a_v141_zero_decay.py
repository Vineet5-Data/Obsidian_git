...
def animal_service_jobs(x, y, tile, step, spot):
    ...
    if held > 0:
        ...
        jobs.append((held * harvest_price * (2.0 if near_cap else 1.0)
                     * terminal + 100.0, (x, y), ["HARVEST"], None))
...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if held > 0 and age >= cd["fyd"]:
        ...
        try:
            ...
            lifespan = -1
        imminent_decay = (0 <= lifespan <= step + 48)

        ready_to_harvest = False
        if cd["ong"]:
            # Harvest ongoing crops (Strawberry/Tomato) immediately to prevent yield cap blockage & decay
            ready_to_harvest = True
        else:
            if age >= cd["myd"] or imminent_decay or step >= DUMP_STEP:
                ready_to_harvest = True
            elif held >= cd["my"]:
                ready_to_harvest = True
        ...
        if ready_to_harvest:
            ...
            base_gain = (held * harvest_price + 100.0) * terminal
            if imminent_decay:
                base_gain += 10000.0  # Emergency priority to prevent decay loss
            gain = base_gain
            if storage_load > 95:
                gain *= 0.1
...
def _plan(obs):
    ...
    # Exclude GOOSE from animal selection - dedicate herd to high-margin COW & SHEEP
    ...
    want_crop = {}
    # Quick wheat creates the opening cash cycle and ongoing feed supply
    if crop_slots > 0 and day <= 26:
        wheat_floor = min(crop_slots, max(6, int(round(crop_slots * (0.40 if day < 3 else 0.35)))))
        current_wheat = own_crop_counts.get("WHEAT", 0) + want_crop.get("WHEAT", 0)
        if current_wheat < wheat_floor:
            add_wheat = min(crop_slots - sum(want_crop.values()), wheat_floor - current_wheat)
            if add_wheat > 0:
    ...
    for idx, pos in units:
        ...
        must_bank = (step >= DUMP_STEP
                     or sale_load >= 12
                     or money < 150.0
                     or (hour >= TPD - 2 and sale_load >= 4))
        ...
        if pos in shed_access_set and sale_load > 0:
