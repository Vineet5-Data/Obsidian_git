...
HOLD_LATE = 0.12
RELAX_DAY = 18
DUMP_STEP = 672          # day 28: cash out while workers can still reach the shed
...
def _plan(obs):
    ...
    for name in sorted(ANIMALS, key=lambda n: -opp_animal_counts.get(n, 0)):
        if name == 'GOOSE' and day > 5: continue
    ...
    crop_slots = max(0, min(65, slots - structure_need))
    want_crop = {}
    ...
    for _ in range(remaining_crop_slots):
        ...
        for crop, cd in CROPS.items():
            if crop == 'CARROT' and day > 10: continue
