...
MAX_BUY_ORDERS = 5
WHEAT_BUY_MAX = 45
MOVE_FRAC = 1.0          # worker-turn opportunity cost, as a share of the
...
def _plan(obs):
    ...
    # The median is a proxy; the real price of spending a worker-turn walking
    # is the best job that therefore goes undone.  With `nfree` workers and a
    # descending job list, that is the first job no worker can reach -- the
    # marginal job.  When jobs are scarce (ncand <= nfree) nothing is displaced
    # and travel is free, which is exactly what an out-of-range index should
    # mean, so clamp to 0.0 rather than to the last job.
    nfree = len(free_units)
    move_rate = (MOVE_FRAC * jobs[nfree][0]) if nfree < ncand else 0.0
    cost, meta = [], []
