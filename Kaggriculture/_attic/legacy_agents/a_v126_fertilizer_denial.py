"""v126 -- convert fertilizer into contested crop pressure instead of draining the shed.

Built after v125 fell to 1025-575 (64.1%).  v125 improved our own farm by
relaxing sell reservations whenever shed occupancy reached 80, but this lifted
STRAWBERRY prices and transferred much more revenue to the opponent than it
created for us.  v126 discards that mechanism and restores the parent's
cash-gated >=80 sell relief.

The new intervention is structurally different:
  1. FERTILIZE is counted as real open work by workforce_target.
  2. For the already-existing strategic STRAWBERRY/TOMATO fertilization branch,
     job value includes the exact one-extra-unit denial imposed on the visible
     opponent wave under the engine's price curve.

This keeps the existing own-production value, adds only a non-negative
head-to-head externality, and does not use opponent identity or replay data.
"""
