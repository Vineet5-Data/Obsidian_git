...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if held > 0 and age >= cd["fyd"]:
        ...
        if storage_load > 90:
            gain *= 0.1
        # A non-ongoing crop is destroyed by its own harvest: HARVEST sets the
        # tile to None (engine :455).  Its yield does not come from the daily
        # refresh -- `_daily_refresh_plants` skips non-ongoing crops outright
        # (:774) -- it comes from WATER, +1 a day inside the age window
        # `(myd+1)//2 .. myd` (:425-430).  WHEAT is fyd 2 / myd 4, so from age 2
        # the same tile simultaneously advertises "harvest me for the 1 free
        # unit planting gave me" and "water me three more times", and whichever
        # fires first ends the tile.  Measured over a full game: our wheat tiles
        # reach 1.85 units on average against the opponent's 3.75 out of a
        # possible 4.00, and wheat is the largest single line in the loss
        # analysis (+$10,409 to the opponent across days 25-29).
        #
        # This deletes a destructive action rather than adding capacity, so it
        # costs no worker turns -- it saves the premature harvest and the
        # replant behind it.  MELON is untouched: its water window opens at
        # age 6 and it is already at max_yield by the time HARVEST unlocks at
        # age 10, so `held < cd["my"]` is already False there.
        still_growing = (not cd["ong"] and age < cd["myd"] and held < cd["my"]
                         and not expiring and step < DUMP_STEP)
