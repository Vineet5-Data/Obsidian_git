"""v138 -- v128 + stateful hidden-rival-wave denial estimator.

v126 is the validated parent at 1125-475 (70.3%). v127 changed fertilizer
carriage but benchmarked identically, so carriage is discarded. The loss graphs
show the actual remaining structure: our opening cash is stronger, the game flips
in days 20-24, and the opponent's dominant revenue edge is STRAWBERRY while our
MELON/WOOL/FERTILIZER remain strengths. Earlier v111 already proved that copying
the opponent's premium wave more exactly is harmful.

The controllable late problem is therefore not "match their strawberries" and
not "do more work". Page-1 action divergence shows the opponent earns more while
PASSing more. v128 changes value per NEW crop commitment, not worker quantity.

Single policy change:
  * run the literal v126 crop allocator unchanged;
  * through day 19, use its result exactly;
  * from day 20 onward, keep the SAME total number of new crop projects requested
    by v126, but reallocate those slots by incremental remaining-horizon terminal
    coins per lifecycle action;
  * value saleable output on the exact shared price curve after forecast town
    absorption and discounted visible opponent supply;
  * value WHEAT needed by the existing herd at avoided BUY_PRODUCT replacement
    cost before valuing any excess as a sale;
  * do not use opponent crop counts/units as a production target and do not add a
    new concentration cap. Market impact self-limits the mix.

Thus the v128 farm policy is retained. v138 adds one isolated market-layer change:
from day 20 onward, public harvest transitions are accumulated into a conservative
estimate of rival stock that has disappeared into the opponent's private carried/shed
inventory. The estimate only changes `opp_wave` used by the already-validated
head-to-head sale/denial objective; it does not change crops, animals, service jobs,
hiring, movement, land, or purchases.
"""
...
# v138: per-seat, per-episode memory of public rival harvests that have disappeared
# from public tile state but may still be sitting in the rival's private inventory.
# This state is deliberately NOT opponent-ID keyed. It is rebuilt from observations.
_RIVAL_WAVE_MEMORY = {}


def reset_rival_wave_memory():
    _RIVAL_WAVE_MEMORY.clear()


def rival_wave_memory_snapshot():
    return {seat: {
        "prev_step": row.get("prev_step"),
        "hidden": dict(row.get("hidden", {})),
        "prev_market": dict(row.get("prev_market", {})),
        "own_prev_sales": dict(row.get("own_prev_sales", {})),
    } for seat, row in _RIVAL_WAVE_MEMORY.items()}


def _rival_public_yield_snapshot(farms, seat):
    """Return public per-tile rival yield state used to detect actual harvests.

    Animal product can only fall through HARVEST. Crop yield drops are counted only
    while the previous tile is still pre-expiry, so ordinary post-lifespan decay is
    not hallucinated into hidden opponent inventory. One-time crop disappearance
    before expiry is treated as harvest; ongoing-crop disappearance is left unknown.
    """
    snap = {}
    if len(farms) < 2:
        return snap
    opponent = farms[1 - seat]
    for y, row in enumerate((_get(opponent, "tiles", []) or [])):
...
def _healthy_crop_at_step(row, step):
    raw = row.get("max_step")
    if raw is None:
        return True
    try:
        return int(step) < int(raw)
    except (TypeError, ValueError):
        return False


def update_rival_hidden_wave(seat, step, farms, market_inventory):
    """Update a conservative lower-resolution estimate of hidden rival sell stock.

    Additions come only from observable harvest-like tile transitions. Removals use
    a *lower bound* on rival sales: a positive shared-market inventory increase above
    the maximum inventory advance our own previous SELL quantity could have caused.
    Town demand and BUY_PRODUCT only lower shared inventory, so ignoring them cannot
    overstate this lower bound.

    The returned estimate is advisory and is applied only from day 20 in `_plan`.
    """
    seat = 1 if int(seat) == 1 else 0
    step = int(step)
    current_market = {k: int(market_inventory.get(k, I0) or I0) for k in PRODUCTS}
    current_snap = _rival_public_yield_snapshot(farms, seat)
    state = _RIVAL_WAVE_MEMORY.get(seat)

    if (state is None or step <= int(state.get("prev_step", -1))
            or step - int(state.get("prev_step", step - 1)) != 1):
        state = {
            "prev_step": step,
            "prev_snapshot": current_snap,
            "prev_market": current_market,
            "own_prev_sales": {},
            "hidden": {item: 0.0 for item in PRODUCTS},
        }
        _RIVAL_WAVE_MEMORY[seat] = state
        return dict(state["hidden"])

    hidden = {item: max(0.0, float(state.get("hidden", {}).get(item, 0.0)))
              for item in PRODUCTS}

    # Reconcile a safe lower bound on stock the rival must have sold last turn.
    prev_market = state.get("prev_market", {})
    own_prev_sales = state.get("own_prev_sales", {})
    for item in PRODUCTS:
        delta = current_market.get(item, I0) - int(prev_market.get(item, I0))
        own_max_advance = max(0, int(own_prev_sales.get(item, 0) or 0))
        rival_sale_lb = max(0, delta - own_max_advance)
        if rival_sale_lb:
            hidden[item] = max(0.0, hidden[item] - rival_sale_lb)

    # Add newly public harvest transitions.
    prev_snap = state.get("prev_snapshot", {})
    for key, before in prev_snap.items():
        held_before = max(0, int(before.get("held", 0) or 0))
        if held_before <= 0:
            continue
        after = current_snap.get(key)
        product = before.get("product")
        if product not in PRODUCTS:
            continue

        if after and after.get("kind") == before.get("kind") \
                and after.get("product") == product:
            drop = held_before - max(0, int(after.get("held", 0) or 0))
            if drop <= 0:
                continue
            if before.get("kind") == "animal":
                hidden[product] += drop
            elif _healthy_crop_at_step(before, state["prev_step"]):
                hidden[product] += drop
        elif (before.get("kind") == "crop" and not before.get("ongoing")
              and _healthy_crop_at_step(before, state["prev_step"])):
            # One-time harvest removes the crop tile. Count only a still-healthy
            # disappearance; expired/rotting tiles are intentionally ignored.
            hidden[product] += held_before

    state["prev_step"] = step
    state["prev_snapshot"] = current_snap
    state["prev_market"] = current_market
    state["hidden"] = hidden
    state["own_prev_sales"] = {}  # filled after this turn's market queue is built
    return dict(hidden)


def remember_own_sales(seat, orders):
    """Remember only quantities; using quantity as max inventory advance is safe."""
    state = _RIVAL_WAVE_MEMORY.get(1 if int(seat) == 1 else 0)
    if state is None:
        return
    sold = {}
    for order in orders or []:
        if isinstance(order, (list, tuple)) and len(order) >= 3 and order[0] == "SELL":
            item = order[1]
            if item in PRODUCTS:
                sold[item] = sold.get(item, 0) + max(0, int(order[2] or 0))
    state["own_prev_sales"] = sold
...
def _plan(obs):
    ...
    # Stateful public-observation estimator. It runs throughout the episode so a
    # day-19 harvest can remain visible to the day-20 denial decision, but it does
    # not affect policy before day 20.
    hidden_rival_wave = update_rival_hidden_wave(seat, step, farms, minv)
    ...
    # v138 sole policy change: after the validated opening, do not let a rival
    # batch become "invisible" merely because it was harvested off the public
    # tile. Preserve v128's exact head-to-head sale objective; improve only Q_r.
    if day >= 20:
        for item in PRODUCTS:
            opp_wave[item] = min(100.0, max(opp_wave[item],
                                           hidden_rival_wave.get(item, 0.0)))
    ...
    # Needed only to reconcile a conservative lower bound on rival sales from
    # the next public market-inventory observation. No policy reads our own log.
    remember_own_sales(seat, orders[:10])
