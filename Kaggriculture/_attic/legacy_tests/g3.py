"""g3 -- pure adaptive policy for Kaggriculture.  No replay tape, no cloned route.
...
policy is built entirely around reacting to it:
  ...
  4. size the workforce to the job list rather than to the calendar,
  5. protect working capital: feed and wages come before new capacity,
  6. size each sell order so its last unit still clears a price floor.
...
MAX_HANDS = 14
JOBS_PER_UNIT = 7.0      # tile-ops one unit completes per day after walking
HOLD_EARLY = 0.86
HOLD_LATE = 0.18
RELAX_DAY = 22
DUMP_STEP = 700
...
def _plan(obs):
    ...
    for crop, cd in CROPS.items():
        ...
        if y:
            options.append(("crop", crop, y[0], y[1], float(cd["seed"]), crop))
    ...
    # workforce sized to the work, not the calendar: this is what kept the
    # earlier build broke -- 14 hands a day costs ~$986 whether needed or not
    open_jobs = sum(1 for v, _t, o, _r in jobs
                    if o[0] in ("WATER", "FEED", "CARE", "HARVEST", "PLANT",
                                "DIG", "BUILD_COOP", "BUILD_PASTURE"))
    want_hands = int(min(MAX_HANDS, math.ceil(open_jobs / JOBS_PER_UNIT)))
    ...
    # working capital: tomorrow's wages plus three days of feed
    reserve = hire_bill(min(want_hands, MAX_HANDS)) * 1.2 + n_beasts * 3.0 * wheat_buy
    reserve = min(reserve, 2500.0) if days_left > 1 else 0.0
    ...
    # feed first -- a starved animal escapes and takes its whole stream with it
    if n_beasts and len(orders) < 10:
        target = n_beasts * FEED_DAYS
        need = max(0, target - shed.get("WHEAT", 0))
        room = max(0, SHED_CAP - shed_used - 4)
        n = min(need, room, int(max(0.0, cash - 40.0) // max(1, wheat_buy)))
        if n > 0:
    ...
    keep_wheat = 0 if step >= DUMP_STEP else min(shed.get("WHEAT", 0),
                                                 n_beasts * FEED_DAYS)
    rows = []
...
if __name__ == "__main__":
    ...
    print("g3 mirror:", int(last[0].reward), int(last[1].reward))
