...
_LOCAL_OPS = {
    "WATER", "HARVEST", "FEED", "CARE", "COLLECT_FERTILIZER",
    "FERTILIZE", "PLANT", "BUILD_COOP", "BUILD_PASTURE", "DIG", "PLACE",
}
        ...
        for x, tile in enumerate(row or []):
            ...
            if kind == "PLANT":
                if not bool(tile.get("watered_today", False)):
                    jobs.append((9400, x, y, "WATER", False))
            elif kind in ("COOP", "PASTURE") and tile.get("animal"):
                if bool(tile.get("fed_today", False)) and not bool(tile.get("cared_today", False)):
                    jobs.append((7600, x, y, "CARE", False))
    return jobs


def _extra_workers(obs, action, base_hand_count, step):
    action = _aligned(action, obs)
    farm, private = _farm_private(obs)
    positions = [_get(farm, "farmer", [0, 0]), *list(_get(farm, "hands", []) or [])]
    inventories = list(_get(private, "inventories", []) or [])
    unit_actions = [action["farmer"], *list(action.get("hands") or [])]
    targets = _shed_targets(farm)
    jobs = _jobs(obs, farm)
    claimed = set()
    for index in range(min(1 + base_hand_count, len(unit_actions))):
        intended = list(unit_actions[index] or ["PASS"])
        if intended and intended[0] in _LOCAL_OPS and index < len(positions):
            claimed.add(_position(positions[index]))

    for index in range(1 + base_hand_count, min(len(unit_actions), len(positions))):
        position = _position(positions[index])
        inventory = dict(inventories[index] or {}) if index < len(inventories) else {}
        cargo = sum(int(value or 0) for key, value in inventory.items() if key != "WHEAT")
        wheat = max(0, int(inventory.get("WHEAT", 0) or 0))
        if int(step) >= int(EXTRA_WORKER_LAST_STEP) and cargo:
            if position in targets:
                unit_actions[index] = ["DROP"]
            elif targets:
                target = min(targets, key=lambda p: abs(p[0] - position[0]) + abs(p[1] - position[1]))
                unit_actions[index] = _step_toward(position, target)
            continue

        feed_waiting = any(job[4] and (job[1], job[2]) not in claimed for job in jobs)
        if feed_waiting and wheat <= 0:
            if position in targets:
                available = max(0, int((_get(private, "shed", {}) or {}).get("WHEAT", 0) or 0))
                unit_actions[index] = ["PICKUP", "WHEAT", min(8, available)] if available else ["PASS"]
            elif targets:
                target = min(targets, key=lambda p: abs(p[0] - position[0]) + abs(p[1] - position[1]))
                unit_actions[index] = _step_toward(position, target)
            continue
        ...
        candidates = []
        for priority, x, y, operation, needs_wheat in jobs:
            if (x, y) in claimed or (needs_wheat and wheat <= 0):
                continue
            distance = abs(x - position[0]) + abs(y - position[1])
            candidates.append((priority - 300 * distance, priority, -distance, -y, -x, x, y, operation))
        if not candidates:
            unit_actions[index] = ["PASS"]
            continue
        _score, _priority, _distance, _ny, _nx, x, y, operation = max(candidates)
        claimed.add((x, y))
        unit_actions[index] = [operation] if position == (x, y) else _step_toward(position, (x, y))
...
def _hire_extras(obs, action):
    action = _aligned(action, obs)
    farm, _private = _farm_private(obs)
    hires = max(0, int(_get(farm, "hires_today", 0) or 0))
    planned = hires + sum(1 for order in action["market"] if order and order[0] == "HIRE")
    money = max(0, int(_get(farm, "money", 0) or 0))
    while planned < int(TARGET_HANDS) and len(action["market"]) < 10:
        cost = _fib(planned)
        if money < cost:
            break
...
def agent(obs):
    try:
        ...
        raw = copy.deepcopy(_ACTIONS[step])
        base_hand_count = len(raw.get("hands") or [])
        action = _aligned(raw, obs)
        action = _extra_workers(obs, action, base_hand_count, step)
        return _hire_extras(obs, action)
    except Exception:
        farms = list(_get(obs, "farms", []) or [])
        seat = 1 if int(_get(obs, "player", 0) or 0) == 1 else 0
        farm = farms[seat] if seat < len(farms) else {}
