"""v138 -- win-probability-oriented timed STRAWBERRY counter, forked from v128.

Validated parent: a_v128_terminal_value.py at 1132-468 (70.8%).

The v125 natural experiment proved that own-score improvement can lose the
match when it lifts a shared STRAWBERRY price and transfers more revenue to the
opponent.  v128 already has an important denial primitive: if the rival has a
visible strawberry position, its sell reservation falls to $1.  The missing
pieces are (a) timing and (b) service valuation.  v128 can sell denial stock too
early, allowing town absorption to restore the market before the opponent wave,
and HARVEST jobs do not explicitly receive the opponent revenue they can deny.

v138 therefore makes one coupled strategic extension -- an intentional
STRAWBERRY denial pipeline -- while preserving the rest of v128:
  * days 0..16 are decision-identical by construction;
  * forecast near-term public exposure from both farms;
  * activate only when a short-horizon relative-revenue forecast is not safely
    winning and the opponent has materially more strawberry exposure;
  * before the rival wave, reserve some strawberry inventory as denial ammo;
  * release it when visible rival yield exists, the next production tick is
    close, storage is stressed, or the wave window is ending;
  * add a discounted opponent-denial bonus to STRAWBERRY HARVEST value so
    workers can create saleable denial inventory when it matters.

No herd-size, crop-mix, shed-drain, worker-count, DIG, fertilizer, land, or
liquidation policy is changed.  This is deliberately a single high-confidence
competitive mechanism suitable for paired screening before any broader merge.
"""
...
# Risk-gated timed-denial controller.  Constants are hypotheses for screening,
# not claimed optima.  The gate is closed before day 17 to protect the parent.
DENIAL_START_DAY = 17
DENIAL_END_DAY = 24
LOCK_WIN_BUFFER = 6000.0
DENIAL_MIN_RATIO = 1.20
DENIAL_MIN_EDGE = 6.0
DENIAL_EXPOSURE_DISCOUNT = 0.80
DENIAL_AMMO_RESERVATION = 0.90
DENIAL_RELEASE_STEPS = 8
DENIAL_STORAGE_RELEASE = 85
DENIAL_HARVEST_REALIZATION = 0.45
...
FERTILIZER_CURRENT_UNIT_MULTIPLIER = 1.15
SHED_SOFT = 50
FEED_CARRY = 5
FERT_CARRY = 4
...
MAX_BUY_ORDERS = 5
WHEAT_BUY_MAX = 45
MOVE_FRAC = 0.8          # worker-turn opportunity cost, as a share of the
                         # median available job value
...
def forecast_near_term_exposure(farm, day, horizon_days, stock=None):
    """Forecast sellable public exposure over the next few in-game days.

    `stock` is supplied for our side only (shed/carried or projected shed).
    Opponent private inventory is never guessed.  Public held yield plus
    scheduled production is sufficient to identify the large premium wave.
    """
    exposure = {name: 0.0 for name in PRODUCTS}
    if stock:
        for item, qty in stock.items():
            if item in exposure:
                exposure[item] += max(0.0, float(qty or 0.0))
        ...
        for tile in (row or []):
            ...
            if tile.get("kind") == "PLANT" and tile.get("crop") in CROPS:
                ...
                if cd["ong"]:
                    ...
                    for ahead in range(1, horizon_days + 1):
                        fday = day + ahead
                        since_first = fday - planted - cd["fyd"]
                        if (since_first >= 0
                                and since_first % interval == 0
                                and since_first // interval < cd["my"]):
                            fert_until = int(tile.get("fertilized_until_day", -1) or -1)
                            exposure[crop] += 1.25 if fert_until >= fday else 1.0
                else:
                    harvest_day = planted + cd["myd"]
                    if day < harvest_day <= day + horizon_days and held <= 0:
                        exposure[crop] += max(1.0, 0.75 * cd["my"])

            elif "animal" in tile and tile.get("animal") in ANIMALS:
                animal = ANIMALS[tile["animal"]]
                item = animal["prod"]
                exposure[item] += max(0, int(tile.get("yield_units", 0) or 0))
                placed = tile.get("placed_day", day)
                placed = day if placed is None else int(placed)
                interval = max(1, animal["iv"])
                for ahead in range(1, horizon_days + 1):
                    fday = day + ahead
                    since_first = fday - placed - animal["fyd"]
                    if since_first >= 0 and since_first % interval == 0:
                        exposure[item] += 1.0
...
def strawberry_wave_timing(farm, day, hour):
    """Return (visible held units, steps to next scheduled strawberry tick)."""
    held = 0.0
    next_steps = None
    ...
    for row in (_get(farm, "tiles", []) or []):
        for tile in (row or []):
            ...
            for ahead in range(0, 8):
                ...
                if (since_first >= 0 and since_first % interval == 0
                        and since_first // interval < cd["my"]):
                    # Production refresh is day-granular.  A due tick today is
                    # treated as immediate; a future-day tick is measured to
                    # the next day boundary.
                    steps = 0 if ahead == 0 else ahead * TPD - hour
                    if next_steps is None or steps < next_steps:
                        next_steps = max(0, steps)
                    break
    return held, next_steps


def symmetric_relative_sale_value(item, inv, ours, rival):
    """Average relative batch revenue over both possible sale orders."""
    ours = max(0, int(math.ceil(float(ours or 0.0))))
    rival = max(0, int(math.ceil(float(rival or 0.0))))
    if item not in MP or (ours <= 0 and rival <= 0):
        return 0.0
    our_first, after_ours = sale_value(item, int(inv), ours)
    rival_late, _ = sale_value(item, after_ours, rival)
    rival_first, after_rival = sale_value(item, int(inv), rival)
    our_late, _ = sale_value(item, after_rival, ours)
    return 0.5 * ((our_first - rival_late) + (our_late - rival_first))


def projected_counter_margin(current_margin, minv, own_exposure, opp_exposure,
                             near_absorb):
    """Short-horizon relative-revenue forecast used only as an intervention gate."""
    margin = float(current_margin)
    for item in PRODUCTS:
        inv = int(minv.get(item, I0) or I0)
        absorbed = max(0, int(math.floor(float(near_absorb.get(item, 0.0) or 0.0))))
        wave_inv = max(0, inv - absorbed)
        margin += symmetric_relative_sale_value(
            item, wave_inv, own_exposure.get(item, 0.0),
            opp_exposure.get(item, 0.0))
    return margin


def strawberry_denial_active(day, projected_margin, own_exposure, opp_exposure):
    if not (DENIAL_START_DAY <= int(day) <= DENIAL_END_DAY):
        return False
    if float(projected_margin) >= LOCK_WIN_BUFFER:
        return False
    ours = max(0.0, float(own_exposure.get("STRAWBERRY", 0.0) or 0.0))
    rival = max(0.0, float(opp_exposure.get("STRAWBERRY", 0.0) or 0.0))
    return rival >= ours * DENIAL_MIN_RATIO + DENIAL_MIN_EDGE


def opponent_denial_value(item, inv, our_qty, rival_qty):
    """Opponent revenue removed by injecting `our_qty` before its batch."""
    our_qty = max(0, int(math.ceil(float(our_qty or 0.0))))
    rival_qty = max(0, int(math.ceil(float(rival_qty or 0.0))))
    if item not in MP or our_qty <= 0 or rival_qty <= 0:
        return 0.0
    _ours, after_ours = sale_value(item, int(inv), our_qty)
    rival_early, _ = sale_value(item, int(inv), rival_qty)
    rival_late, _ = sale_value(item, after_ours, rival_qty)
    return max(0.0, float(rival_early - rival_late))


def make_strawberry_denial_context(day, hour, money, opponent_money, minv,
                                   shops, own_exposure, opp_exposure,
                                   opponent, storage_load):
    horizon_days = max(1, min(5, DENIAL_END_DAY - int(day) + 1))
    near_absorb = forecast_absorption(shops, horizon_days * TPD)
    counter_margin = projected_counter_margin(
        float(money) - float(opponent_money), minv,
        own_exposure, opp_exposure, near_absorb)
    active = strawberry_denial_active(
        day, counter_margin, own_exposure, opp_exposure)
    held, next_steps = strawberry_wave_timing(opponent, day, hour)
    release = bool(active and (
        held >= 1.0
        or (next_steps is not None and next_steps <= DENIAL_RELEASE_STEPS)
        or int(day) >= DENIAL_END_DAY
        or int(storage_load) >= DENIAL_STORAGE_RELEASE
        or float(money) < 500.0))
    rival = max(0.0, float(opp_exposure.get("STRAWBERRY", 0.0) or 0.0))
    return {
        "active": active,
        "release": release,
        "projected_margin": counter_margin,
        "visible_held": held,
        "next_wave_steps": next_steps,
        "rival_exposure": DENIAL_EXPOSURE_DISCOUNT * rival,
    }
...
def animal_allocation_limits(day, days_left, quadrant_count, slots,
    ...
    """Return v110's herd target, new-animal need, and structure reservation."""
...
def rank_sales(projected, minv, opp_wave, sell_hold, keep_wheat, step,
    ...
    With no active denial context this is decision-identical to v128.  In an
    active risk state, strawberry stock is temporarily treated as denial ammo
    until the rival monetization wave is close; on release, the rival batch in
    the externality term is expanded from the next-tick proxy to the discounted
    near-term public exposure.
    """
    ctx = denial_context or {}
    rows = []
    for item, raw_qty in projected.items():
        ...
        if qty <= 0:
            continue
        baseline_rival = int(math.ceil(max(0.0, opp_wave.get(item, 0.0))))
        active_straw = bool(item == "STRAWBERRY" and ctx.get("active"))
        release_straw = bool(active_straw and ctx.get("release"))
        rival_qty = baseline_rival
        if release_straw:
            rival_qty = max(
                baseline_rival,
                int(math.ceil(max(0.0, float(ctx.get("rival_exposure", 0.0))))))
        ...
        if sell_hold <= 0.0:
            ...
            if item == "STRAWBERRY" and baseline_rival > 0:
                if active_straw and not release_straw:
                    # Preserve denial inventory until the visible rival wave is
                    # close enough that town absorption cannot undo the crash.
                    reservation = max(
                        reservation, DENIAL_AMMO_RESERVATION * MP[item]["base"])
                    rival_qty = 0
                else:
                    reservation = 1.0
...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if held > 0 and age >= cd["fyd"]:
        terminal = 5.0 if step >= DUMP_STEP else 1.0
        # HARVEST moves yield tile -> shed; it is not a sale.  rank_sales
        # already refuses to sell below hold_fraction * base, so a crashed
        # spot is a reason to hold in the shed, not to leave yield on the
        # plant.  Unharvested yield is destroyed either way: an at-cap
        # ongoing crop discards its next production (engine min(max_yield)),
        # and an expired tile rots one unit every two steps.  Value the
        # harvest at the reservation price we would insist on when selling,
        # which decays to ~0 by the last days on its own.
        # Only yield the engine is about to destroy justifies overriding the
        # gate.  Yield that can simply wait on the plant still waits, exactly
        # as in v122, so the strong days 0-19 phase is left alone.
        if (product_price < 50
            ...
            if expiring or at_cap:
                harvest_price = max(
                    product_price,
                    hold_fraction(step, day, storage_load)
                    * MP[tile["crop"]]["base"])
            else:
                harvest_price = 0.0
        else:
            harvest_price = product_price
        gain = (held * harvest_price + (14 if not cd["ong"] else 0)) * terminal
        if storage_load > 90:
            gain *= 0.1

        # Competitive value: a harvested strawberry can be denial inventory,
        # not merely our own future sale.  Discount heavily because harvesting
        # still requires return/drop/sale execution before the externality is
        # realized.  This branch is closed outside the risk-gated wave window.
        ctx = denial_context or {}
        if (tile.get("crop") == "STRAWBERRY" and held > 0
                and ctx.get("active") and market_inv is not None):
            rival_qty = int(math.ceil(max(0.0, float(ctx.get("rival_exposure", 0.0)))))
            denial = opponent_denial_value(
                "STRAWBERRY", market_inv.get("STRAWBERRY", I0), held, rival_qty)
            gain += DENIAL_HARVEST_REALIZATION * denial
    ...
    if (step < FINAL_FARM_STEP and cd["ong"]
        ...
        if due and (incremental > 12.0 or strategic_denial):
            fertilizer_jobs = 1
            denial_bonus = 0.0
            # The benchmark evidence is specifically a STRAWBERRY denial leak.
            # Keep TOMATO's existing strategic branch byte-for-byte in value.
...
def _plan(obs):
    ...
    # ---- competitive comeback-risk context -------------------------------
    # Use current shed+carried stock for service valuation.  Sales recompute
    # the same context after this turn's deposits are projected.
    counter_horizon_days = max(1, min(5, DENIAL_END_DAY - day + 1))
    own_counter_stock = {k: int(shed.get(k, 0) or 0) + int(carried_all.get(k, 0) or 0)
                         for k in PRODUCTS}
    own_near_exposure = forecast_near_term_exposure(
        farm, day, counter_horizon_days, own_counter_stock)
    opp_near_exposure = forecast_near_term_exposure(
        opponent, day, counter_horizon_days, None)
    denial_context = make_strawberry_denial_context(
        day, hour, money, float(_get(opponent, "money", 0) or 0), minv, shops,
        own_near_exposure, opp_near_exposure, opponent,
        shed_used + sum(carried_all.values()))
    ...
    for x, y, t in plants:
        tile_jobs, tile_fertilize = crop_service_jobs(
            ...
            opp_crop_counts, opp_wave, minv, denial_context)
        jobs.extend(tile_jobs)
    ...
    # Recompute our known exposure with post-action projected shed stock before
    # deciding whether to hold or release denial ammunition this turn.
    own_sale_exposure = forecast_near_term_exposure(
        farm, day, counter_horizon_days, projected)
    sale_denial_context = make_strawberry_denial_context(
        day, hour, money, float(_get(opponent, "money", 0) or 0), minv, shops,
        own_sale_exposure, opp_near_exposure, opponent, projected_used)
...
if __name__ == "__main__":
    ...
    print("v126 mirror:", int(last[0].reward), int(last[1].reward))
