...
def animal_allocation_limits(day, days_left, quadrant_count, slots,
    ...
    """Return optimal 14-animal herd target (8 Cows, 6 Sheep)."""
...
def _plan(obs):
    ...
    if day < 3 and crop_slots > 0:
        straw_target = min(crop_slots, 42)
        want_crop["STRAWBERRY"] = straw_target
        sim_supply["STRAWBERRY"] = sim_supply.get("STRAWBERRY", 0.0) + 8 * straw_target
        quick_wheat = min(crop_slots - straw_target, 12)
        if quick_wheat > 0:
            want_crop["WHEAT"] = quick_wheat
            sim_supply["WHEAT"] = sim_supply.get("WHEAT", 0.0) + 4 * quick_wheat
    ...
    for _ in range(remaining_crop_slots):
        ...
        for crop in crop_candidates:
            ...
            score = gain / (max(1.0, y[1]) * time_risk * (1.0 + 0.040 * crowd))
            if best is None or score > best[0]:
                best = (score, crop, y[0])
        if best is None:
            break
