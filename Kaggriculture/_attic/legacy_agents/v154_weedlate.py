...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if step < FINAL_FARM_STEP and not tile.get("watered_today") and not doomed:
        ...
        if int(tile.get("consecutive_unwatered", 0) or 0) >= 1:
            # A weed is permanent loss of farm area, worth 50,000 only while
            # there is still game left to farm it.  After the cut the tile will
            # never be replanted and the stock a weed destroys is already
            # priced by this tile's own HARVEST job, so the constant becomes the
            # same kind of enormous fake top-priority signal as the +1000
            # capped-tile WATER bonus.
            if step < DUMP_STEP:
                gain += 50000.0
            gain += 3.0 * product_price * max(1, cd["my"] - held)
        if gain > 0:
