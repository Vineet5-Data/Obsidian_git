...
FEED_CARRY = 5
FERT_CARRY = 4
HUNGARIAN_JOBS = 200      # columns in the worker-to-job assignment matrix
FEED_DAYS = 1
FEED_BUFFER = 0.80
MAX_BUY_ORDERS = 5
WHEAT_BUY_MAX = 45
...
def crop_yield(cd, days_left):
    ...
    span = min(cd["myd"], days_left)
    if span < cd["fyd"]:
        return None
    waters = max(0, span - (cd["myd"] + 1) // 2 + 1)
    return min(cd["my"], 1 + waters), 2 + waters, span
...
def crop_season(cd, days_left):
    ...
    if cd["ong"]:
        ...
        return units, actions, float(cd["seed"])
        
    full_cycles = int(days_left // cd["myd"])
    rem_days = days_left - (full_cycles * cd["myd"])
    
    full_y = crop_yield(cd, cd["myd"])
    total_units = full_cycles * full_y[0]
    total_actions = full_cycles * full_y[1]
    
    rem_y = crop_yield(cd, rem_days)
    if rem_y:
        total_units += rem_y[0]
        total_actions += rem_y[1]
        
    return total_units, total_actions, float(cd["seed"]) * (full_cycles + (1 if rem_y else 0))
...
def _plan(obs):
    ...
    want_animal = {}
    species_cap = max(2, int(math.ceil(max(1, animal_target) * 0.70)))
    ...
    for _ in range(remaining_crop_slots):
        ...
        for crop, cd in CROPS.items():
            ...
            if best is None or score > best[0]:
                best = (score, crop, y[0])
        if best is None:
            # Wheat internal value is high (feed), so fill remaining slots with Wheat to save $14/unit
            cd = CROPS.get("WHEAT")
            if cd and days_left >= cd["fyd"]:
    ...
    for x, y, t in beasts:
        ...
        if held > 0:
            ...
            _p = 0.0 if (p < 50 and a["prod"] in ("MELON", "STRAWBERRY", "MILK", "WOOL")) else p
            jobs.append((held * _p * (2.0 if near_cap else 1.0) * terminal,
                         (x, y), ["HARVEST"], None))
        if t.get("fertilizer_available"):
    ...
    for x, y, t in plants:
        ...
        if held > 0 and age >= cd["fyd"]:
            ...
            batch_tier = 0.0
            _p = 0.0 if (p < 50 and t["crop"] in ("MELON", "STRAWBERRY", "MILK", "WOOL")) else p
            gain = batch_tier + (held * _p + (14 if not cd["ong"] else 0)) * terminal
            if shed_used + sum(carried_all.values()) > 90:
                gain *= 0.1
    ...
    for crop, n_want in sorted(want_crop.items(), key=lambda kv: -kv[1]):
        ...
        if not y:
            continue
        v = marginal(crop)
        if crop == "WHEAT" and v < 14.0:
            v = 14.0
        per = (y[0] * v - y[2])
        limit = min(n_want + 4, seeds.get(crop, 0))
        if crop == "WHEAT":
            limit = min(len(crop_empties), seeds.get(crop, 0))
        for x, yy in crop_empties[:limit]:
            jobs.append((min(200.0, max(5.0, per)), (x, yy), ["PLANT", crop], None))
        crop_empties = crop_empties[limit:]

    if days_left >= 2 and step < DUMP_STEP:
        for x, y in weeds:
            jobs.append((200.0, (x, y), ["DIG"], None))
    ...
    for item, raw_qty in projected.items():
        ...
        if sell_hold <= 0.0:
            ...
            reservation = max(1.0, (0.0 if step >= DUMP_STEP else sell_hold) * MP[item]["base"])
    ...
    computed_hands = int(math.ceil(open_jobs / per_hand)) if open_jobs else 0
    if open_jobs and day < LAST_DAY:
        floor_hands = min(10, 6 + footprint // 15 + min(2, len(development) // 12))
    elif open_jobs:
        floor_hands = min(7, max(2, int(math.ceil(open_jobs / 3.0))))
    else:
        floor_hands = 0
    ...
    if True:
        ...
        for crop, desired in want_crop.items():
            y = crop_season(CROPS[crop], days_left)
            if not y:
                continue
            v = marginal(crop)
            if crop == "WHEAT" and v < 14.0:
                v = 14.0
            unit_value = y[0] * v - y[2]
            crop_rank.append((unit_value, desired, crop))
        crop_rank.sort(reverse=True)
        
        # Seed Buffer: maintain a stockpile of seeds, but only AFTER we've expanded
        # to 3 quadrants.
        if len(quads) >= 3:
            seed_buffer = {"WHEAT": 40, "STRAWBERRY": 20, "MELON": 10}
            for crop, target_buffer in seed_buffer.items():
                if crop not in CROPS: continue
                current = int(seeds.get(crop, 0) or 0)
                if current < target_buffer:
                    buy_amt = min(16, target_buffer - current)
                    c = CROPS[crop]["seed"]
                    if cash - c * buy_amt >= reserve and len(orders) < 10 and days_left >= CROPS[crop]["fyd"]:
                        orders.append(["BUY_SEED", crop, buy_amt])
                        cash -= c * buy_amt
                        seeds[crop] = current + buy_amt
        for _value, desired, crop in crop_rank:
            if len(orders) >= 10 or purchase_orders >= MAX_BUY_ORDERS:
                break
            c = CROPS[crop]["seed"]
            need = max(0, desired - seeds.get(crop, 0))
            n = min(need, 16, int(max(0.0, cash - reserve) // c))
            if n <= 0:
                continue
