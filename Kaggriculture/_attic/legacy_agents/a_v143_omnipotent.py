...
def adjusted_sell_hold(base_hold, cash, projected_used, n_beasts,
    ...
    if projected_used > SHED_SOFT:
        sell_hold = min(sell_hold, max(0.08, 0.45 * (1.0 - projected_used / 75.0)))
    ...
    for item, raw_qty in projected.items():
        ...
        if sell_hold <= 0.0:
            ...
            if item == "STRAWBERRY" and rival_qty > 0:
                reservation = 1.0
            # Retain fertilizer for ongoing Strawberry crops instead of dumping raw fertilizer
            if item == "FERTILIZER" and step < DUMP_STEP and qty <= 15:
                continue
            rival_early, _ = sale_value(item, minv.get(item, I0), rival_qty)
            best_sale = (0.0, 0)
            max_qty_to_sell = qty if step >= DUMP_STEP else min(qty, 4)
            for candidate in range(1, max_qty_to_sell + 1):
...
def workforce_target(jobs, day, hour, footprint):
    """Return desired hands capped at economic optimum (max 10/day, max 8/day late)."""
    ...
    if open_jobs and day < LAST_DAY:
        ...
        floor = 0
    day_cap = 8 if day >= 22 else 10
    return min(day_cap, max(computed, floor)), summary
...
def animal_service_jobs(x, y, tile, step, spot):
    ...
    if held > 0:
        ...
        harvest_price = max(float(product_price), 0.5 * MP[animal["prod"]]["base"])
        jobs.append((held * harvest_price * (2.0 if near_cap else 1.0)
                     * terminal + (200.0 if near_cap else 50.0), (x, y), ["HARVEST"], None))
...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if held > 0 and age >= cd["fyd"]:
        ...
        try:
            ...
            lifespan = -1
        expiring = (0 <= lifespan <= step + 2 * TPD)
        at_cap = cd["ong"] and held >= cd["my"]

        harvest_price = max(float(product_price), 0.5 * MP[tile["crop"]]["base"])
        base_gain = (held * harvest_price + (14 if not cd["ong"] else 50.0)) * terminal
        if expiring:
            base_gain += 5000.0  # Emergency priority to guarantee collection before decay
        gain = base_gain
        if storage_load > 95 and not expiring:
            gain *= 0.1
    ...
    if (step < FINAL_FARM_STEP and cd["ong"]
        ...
        if due and (incremental > 5.0 or strategic_denial or crop_name == "STRAWBERRY"):
            fertilizer_jobs = 1
            denial_bonus = 0.0
...
def _plan(obs):
    ...
    # fallback. Only consider Goose if an Egg-demanding shop is active.
    egg_shops = {"BRUNCH_SPOT", "BAKERY", "PET_CAFE"}
    livestock_candidates = [n for n in ANIMALS if n != "GOOSE" or any(s in egg_shops for s in shops)]
    for name in sorted(livestock_candidates, key=lambda n: -opp_animal_counts.get(n, 0)):
    ...
    for _ in range(max(0, animal_need - sum(want_animal.values()))):
        best = None
        for name in livestock_candidates:
            a = ANIMALS[name]
            y = animal_yield(a, days_left)
            if not y:
                continue
    ...
    wheat_demanding_shops = sum(1 for s in shops if "WHEAT" in SHOPS.get(s, []))
    if day >= 3 and wheat_demanding_shops >= 2 and crop_slots > 0:
        wheat_target = min(crop_cap, int(crop_slots * (0.35 + 0.08 * (wheat_demanding_shops - 1))))
        deficit = max(0, wheat_target - own_wave_ready_counts.get("WHEAT", 0))
        n = min(crop_slots - sum(want_crop.values()), deficit)
        if n > 0:
            want_crop["WHEAT"] = want_crop.get("WHEAT", 0) + n
            own_wave_ready_counts["WHEAT"] = own_wave_ready_counts.get("WHEAT", 0) + n
            sim_supply["WHEAT"] = sim_supply.get("WHEAT", 0.0) + n * CROPS["WHEAT"]["my"]
    ...
    if hour <= HIRE_WINDOW and open_jobs and len(orders) < 10:
        ...
        max_hire_allowed = min(8 if day >= 22 else 10, want_hands)
        while (n_hired < max_hire_allowed and len(orders) < 10
