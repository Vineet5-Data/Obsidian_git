...
def _plan(obs):
    ...
    for _ in range(remaining_crop_slots):
        ...
        for crop, cd in CROPS.items():
            ...
            effective_cap = crop_cap
            if crop == "STRAWBERRY" and minv.get("STRAWBERRY", I0) <= I0:
                effective_cap = max(crop_cap, int(math.ceil(max(1, portfolio_size) * 0.70)))
            if already >= effective_cap:
                continue
