...
for lead in (0, 2, 4, 6):
    ...
    env.run([load("v22.py", dict(SELL_LEAD=lead)).agent, make_replay_agent(RP, player=0)])
    f = env.steps[-1]; us, them = f[0]["reward"], f[1]["reward"]
    print("v22 SELL_LEAD=%-2d ours=$%-9.0f seb=$%-9.0f %s (%+.0f)" % (lead, us, them, "WIN" if us > them else "loss", us-them), flush=True)
