...
def _plan(obs):
    ...
    # `owned_extra < 2` stopped at the second parcel, so LAND_PRICES[2] (4000)
    # was unreachable and the `owned_extra >= 2` branch of se_gate below could
    # never execute -- the expensive-parcel gate was written and then blocked.
    # Loss analysis: land spend is exactly 3,000 (1000+2000) for BOTH sides, so
    # the whole fourth quadrant, 25 tiles, is sitting unclaimed every game.
