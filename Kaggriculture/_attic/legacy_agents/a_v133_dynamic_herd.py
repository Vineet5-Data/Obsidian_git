...
def animal_allocation_limits(day, days_left, quadrant_count, slots,
                             beast_count, struct_count, pending_animals, cash):
    ...
    elif day >= 8 and day < 18:
        affordable = max(0, int((cash - 2000.0) // 500.0))
        target = min(16, current_assets + affordable)
    elif day < 15:
        target = 16
    else:
        target = min(20, max(6, int(round(25 * quadrant_count * 0.22))))
    
    target = max(current_assets, target)
...
def _plan(obs):
    ...
    # v132: v128 has already decided HOW MANY economically/strategically
    # authorized animal slots it wants. Reallocate only those purchasable slots
    # by residual terminal economics; do not change herd size. Desired species
    # that cannot mature before the horizon would be skipped by v128's purchase
    # loop anyway, so exclude them from the conserved slot count.
    legacy_want_animal = dict(want_animal)
    legacy_purchase_slots = sum(
        max(0, int(n or 0))
        for name, n in legacy_want_animal.items()
        if animal_yield(ANIMALS[name], days_left) is not None)
    if legacy_purchase_slots > 0:
        want_animal, sim_supply = allocate_residual_species(
            legacy_purchase_slots, days_left, species_cap, own_animal_counts,
            shed, carried_all, supply, known_absorb, minv)
