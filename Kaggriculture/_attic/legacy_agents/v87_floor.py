...
MAX_BUY_ORDERS = 5
WHEAT_BUY_MAX = 45
MOVE_FRAC = 0.5          # worker-turn opportunity cost, as a share of the
                         # median available job value

# Measured dead weight in the 2026-08-10 Kaggle loss replays.  Over 9 episodes
# we planted 166 CARROT (market price ~38) to opponents' 12, and built 27 COOPs
# for EGG (~55) while every opponent built zero -- a COW's MILK sells at ~259
# off the same PASTURE tile and the same one-wheat-a-day feed.  Both lose to
# simply using the tile for something else.  Kept as a named set rather than
# deleted from CROPS/ANIMALS so the market and opponent-modelling code can still
# price them, and so re-enabling is a one-line change if a board ever makes them
# pay (PET_CAFE / FARMERS_MARKET can spike CARROT).
SKIP_CROPS = ("CARROT",)
SKIP_ANIMALS = ("GOOSE",)

# Job acceptance floor, in net job value (job value minus the travel toll).
#
# The Hungarian solver always hands every free unit *some* job, however far and
# however worthless -- there was no option to decline.  So a unit with nothing
# good nearby would spend three turns walking to a 5-point job.  Top-player
# replays (91566965, 91569740) show the opposite discipline: they PASS ~1,145
# times an episode to our ~431, and get 0.99-1.03 moves per unit of real work
# where we spend 1.89.  They do MORE work (2,845 vs 2,627) with 40% FEWER moves.
# Standing still beats walking to a job that does not pay for the walk.
PASS_FLOOR = 0.0
...
def _plan(obs):
    ...
    for _s, idx, j, d in pairs:
        if idx in busy or j in done:
            continue
        # `_s` is the net value the assignment matrix scored this pairing at:
        # job value minus MOVE_FRAC * distance.  Below the floor the walk costs
        # more than the job returns, so decline it and let the unit fall through
        # to the idle chain (which ends in PASS) rather than commute for nothing.
        if _s < PASS_FLOOR:
            continue
        value, tgt, op, req = jobs[j]
        op = list(op)
