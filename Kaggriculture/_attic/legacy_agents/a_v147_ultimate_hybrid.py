...
def crop_season(cd, days_left):
    ...
    # in practice. We use a 1.80x multiplier to reflect realistic fertilized yields.
    yield_multiplier = 1.80 if cd["ong"] else 1.0
...
def rank_sales(projected, minv, opp_wave, sell_hold, keep_wheat, step):
    ...
    for item, raw_qty in projected.items():
        ...
        if sell_hold <= 0.0:
            ...
            if item == "FERTILIZER" and step < DUMP_STEP and qty <= 10:
                continue
...
def _plan(obs):
    ...
    livestock_candidates = [n for n in ANIMALS if n != "GOOSE"]
    ...
    # Dynamic Wheat scaling based on active town shops demanding Wheat
    wheat_shops = sum(1 for s in shops if "WHEAT" in SHOPS.get(s, []))
    ...
    elif wheat_shops >= 2 and crop_slots > 0 and day <= 26:
        wheat_ratio = min(0.45, 0.25 + 0.06 * (wheat_shops - 1))
        wheat_target = min(crop_slots, max(6, int(round(crop_slots * wheat_ratio))))
        current_wheat = own_crop_counts.get("WHEAT", 0) + want_crop.get("WHEAT", 0)
        if current_wheat < wheat_target:
            add_wheat = min(crop_slots - sum(want_crop.values()), wheat_target - current_wheat)
            if add_wheat > 0:
                want_crop["WHEAT"] = want_crop.get("WHEAT", 0) + add_wheat
                sim_supply["WHEAT"] = sim_supply.get("WHEAT", 0.0) + 6 * add_wheat

    portfolio_size = len(plants) + crop_slots
    crop_cap = max(3, int(math.ceil(max(1, portfolio_size)
                                    * (0.45 if len(shops) < 2 else 0.55))))
    # Compete for fragile premium markets - prioritize STRAWBERRY & MELON over TOMATO
    ...
    crop_candidates = [c for c in CROPS if c != "TOMATO"]
    for _ in range(remaining_crop_slots):
        ...
        for crop in crop_candidates:
            ...
            score = gain / (max(1.0, y[1]) * time_risk * (1.0 + 0.050 * crowd))
            if best is None or score > best[0]:
                best = (score, crop, y[0])
        if best is None:
            break
