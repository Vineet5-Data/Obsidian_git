...
MAX_HANDS = 12
HIRE_WINDOW = 9
JOBS_PER_UNIT = 8.0      # open jobs each hired unit is expected to cover
                         #
                         # Was 4.0.  Raising it to 8.0 is the single largest
                         # measured gain of the project: 65.6% win rate over
                         # 180 panel games vs 48.9% for the 4.0 version, and
                         # mean margin +3,675 vs -1,239.
                         #
                         # It works by hiring FEWER hands (~10 rather than ~12).
                         # Kaggle replays show top players running ~9.7 units
                         # (HIRE 275-287 orders/episode to our 326) and getting
                         # ~1.0 moves per unit of real work where we spend 1.9:
                         # concentrating the same work into fewer units puts
                         # each unit's next job closer, instead of spreading
                         # thin and commuting.
                         #
                         # Effective ceiling is ~6.9, not 8.0 -- per_hand is
                         # min(JOBS_PER_UNIT, hours_available * 0.30) and
                         # hours_available peaks at 23.  Measured: 8.0, 10.0,
                         # 12.0 and 16.0 all produce byte-identical results
                         # (118-62), so anything above ~7 is the same agent.
                         #
                         # Do not push hand count lower by other means -- that
                         # was tested and it collapses: floor_hands 8 -> 18.9%,
                         # floor_hands 7 -> 43.3%, MAX_HANDS 10 -> 63.3%.
                         # The floor of 10 is load-bearing.
HOLD_EARLY = 0.62
HOLD_LATE = 0.12
RELAX_DAY = 18
    ...
    for x, y, t in plants:
        ...
        if (step < FINAL_FARM_STEP
            ...
            if cd["ong"]:
                ...
                # The engine gives every crop the x2 fertilizer bonus, not just
                # ongoing ones (kaggriculture.py:780-784), inside the window
                #     (max_yield_day + 1) // 2 <= age_days <= max_yield_day
                # The old cd["ong"] gate skipped these, so MELON (base 250) was
                # never fertilized.  Reference tier-9 closer_cleo FERTILIZEs 103
                # times a season and Kaggle top players 57-70; we manage 44.
                age = day + 1 - planted_day
                due = ((cd["myd"] + 1) // 2) <= age <= cd["myd"]
