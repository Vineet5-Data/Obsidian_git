...
_JOB_RERANK = {}   # v207: proof the objective-aligned admission fires
_TELEMETRY_ACTIVE = {}
...
def _plan(obs):
    ...
    # v207: admit jobs on the objective the matcher actually optimises.
    #
    # `jobs` is value-sorted (see the sort above) and then truncated to
    # HUNGARIAN_JOBS columns, but the cost matrix below scores
    # `value - move_rate * d`.  Ranking the board by raw value therefore
    # evicts a zero-travel job worth 300 in favour of 48 jobs worth 301 that
    # sit eight tiles away and score deeply negative once the walk is charged.
    # Telemetry: movement is 152-167 worker-turns/game-day across days 15-29
    # against ~150 productive ones -- half of all late-game labour is walking,
    # while PASS has already fallen to ~1/day, so travel is the binding cost.
    #
    # top_avg / move_rate are still calibrated off the VALUE ordering, which is
    # what "opportunity cost of a worker-turn" means.  Only admission changes:
    # a job is ranked by its best case over the free workers, which is an upper
    # bound on any score it could win in the matrix -- a job that cannot beat
    # the cut at its nearest worker cannot beat it at any of them.
    if move_rate > 0.0 and free_units and len(jobs) > ncand:
        _free_pos = [pos for _idx, pos in free_units]
        jobs.sort(key=lambda j: -(j[0] - move_rate
                                  * min(dist(pos, j[1]) for pos in _free_pos)))
        _JOB_RERANK["steps"] = _JOB_RERANK.get("steps", 0) + 1
        _JOB_RERANK["dropped"] = _JOB_RERANK.get("dropped", 0) + (len(jobs) - ncand)
