...
our_agent = "a_v142_surgical_dominance.py"
opponents = [
...
results = {"v142 Surgical Dominance": 0}
...
for opp_name, opp_path in opponents:
    ...
    print(f"\n--- Match: v142 Surgical Dominance (P1) vs {opp_name} (P2) ---")
    env = make("kaggriculture", configuration={"episodeSteps": 720})
    try:
        env.run([our_agent, opp_path])
        final = env.steps[-1]
        r1, r2 = final[0].reward, final[1].reward
        print(f"Result: v142: {r1}, {opp_name}: {r2}")
        if r1 > r2:
            results["v142 Surgical Dominance"] += 1
        elif r2 > r1:
            results[opp_name] += 1
    except Exception as e:
        print(f"Error running match: {e}")

    print(f"\n--- Match: {opp_name} (P1) vs v142 Surgical Dominance (P2) ---")
    env = make("kaggriculture", configuration={"episodeSteps": 720})
    try:
        env.run([opp_path, our_agent])
        final = env.steps[-1]
        r1, r2 = final[0].reward, final[1].reward
        print(f"Result: {opp_name}: {r1}, v142: {r2}")
        if r2 > r1:
            results["v142 Surgical Dominance"] += 1
        elif r1 > r2:
            results[opp_name] += 1
