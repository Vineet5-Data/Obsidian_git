...
def _plan(obs):
    ...
    # Disjoint crop blocks.  Every crop used to be offered the SAME
    # `crop_empties[:n]` prefix, so crops competed for identical tiles and
    # interleaved tile-by-tile: neighbouring tiles ripened on different days and
    # a unit could never work two in a row without walking.  That is the 1.6
    # moves-per-productive-action tax in the Kaggle loss replays (opponents ~1.0).
    #
    # A cursor hands each crop its own slice -- the same idiom the animal siting
    # above already uses -- so a run of tiles ripens together and is worked in
    # place.  The distance-to-shed ordering of `empties` is deliberately left
    # alone: the shed is at the board centre, so that ordering is what keeps the
    # harvest-to-shed leg short.
    crop_cursor = 0
    for crop, n_want in sorted(want_crop.items(), key=lambda kv: -kv[1]):
        ...
        # +4 overhang keeps candidates if tiles in the slice fall through; the
        # cursor advances by n_want so slices stay essentially disjoint.
