...
def main():
    ...
    jobs = [(r, o, s, seat) for r in candidates for o, _ in TARGETS
            if os.path.exists(o) for s in seeds for seat in (0, 1)]
    print(f"{len(candidates)} routes x {len(TARGETS)} target opponents x {n} "
          f"seeds x 2 seats = {len(jobs)} games", flush=True)
