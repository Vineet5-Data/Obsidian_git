...
def animal_service_jobs(x, y, tile, step, day, storage_load, spot):
    ...
    if held > 0:
        ...
        if (product_price < 50
                and animal["prod"] in ("MELON", "STRAWBERRY", "MILK", "WOOL")):
            # A capped animal discards its whole daily tick, exactly as a capped
            # crop tile does, so `crop_service_jobs` prices held stock at the
            # hold value rather than 0 here.  Animals had no such branch: the
            # HARVEST value stayed 0.0 and the near_cap 2.0x urgency multiplied
            # into it, so the animal never got emptied and never produced again.
            harvest_price = (
                max(float(product_price),
                    hold_fraction(step, day, storage_load)
                    * MP[animal["prod"]]["base"])
                if held >= animal["mh"] else 0.0)
        else:
...
def _plan(obs):
    ...
    fertilize_needed = 0
    for x, y, t in beasts:
        tile_jobs, tile_unfed = animal_service_jobs(
