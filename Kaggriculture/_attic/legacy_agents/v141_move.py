...
MOVE_STAT = "mean"       # "mean" reproduces the parent exactly
MOVE_SPIKE = 1000.0      # above this a job value is a priority signal, not cash
_SCALE_TRACE = []        # diagnostics only; never read by a decision


def job_board_scale(jobs, ncand, n_free):
    """Opportunity cost of one worker-turn, read off this turn's own job board.

    The parent averages the top `n_free` values.  That is not what a worker-turn
    is worth: WATER injects a flat 50,000 for any tile facing its second dry day
    (two misses kill the tile), so a couple of emergencies drag the average into
    the thousands, price every job at distance >= 1 below worthless, and reduce
    the Hungarian to "walk to the nearest tile" -- 51% of all worker-turns are
    movement, against a PASS rate of 1.6/day.

    The statistic is selectable because changing it also moves MOVE_FRAC off its
    calibration; the two have to be swept together.
    """
    if ncand <= 0:
        return 0.0
    if MOVE_STAT == "mean":
        n = min(ncand, max(1, n_free))
        return sum(jobs[i][0] for i in range(n)) / n
    values = [jobs[j][0] for j in range(ncand)]
    if MOVE_STAT == "trimmed":
        keep = [v for v in values if v < MOVE_SPIKE]
        if keep:
...
def _plan(obs):
    ...
    move_rate = MOVE_FRAC * job_board_scale(jobs, ncand, len(free_units))
    cost, meta = [], []
