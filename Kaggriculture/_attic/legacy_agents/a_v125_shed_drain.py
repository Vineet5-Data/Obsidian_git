"""v125 -- relieve shed pressure on storage alone, not only on low cash.
...
The sole behavioral change from validated v114 is COLLECT_FERTILIZER scoring
during days 15-19. A ready fertilizer unit already has v114's current-unit
value; collecting it also frees the animal's one-unit buffer for the next
production opportunity. The v118 benchmark showed that this correction raised
cash through day 19, while leaving it active after day 20 displaced PLANT,
WATER, and HARVEST work and reduced late cash. v119 therefore preserves the
early correction and restores v114 scoring from day 20 onward. All other
decisions remain v114-identical. v117's behavior-neutral telemetry is retained
so the benchmark can verify the phase ablation.

v122 added horizon-aware partial harvests for one-time crops: when
max_yield_day runs past the season, harvest at min(max_yield_day, days_left)
provided that day is not before first_yield_day.  Benchmarked 1058-542
(66.1%), beating v119's 1049-551.

v123 is the sole behavioral change from v122, in crop_service_jobs only.
v122 scored a crop HARVEST job at 0.0 whenever the product's spot price fell
below 50 and the crop was MELON or STRAWBERRY.  Because STRAWBERRY is an
ongoing crop it earns no one-time replant bonus, so its harvest job value
became exactly zero and the job was never scheduled.  STRAWBERRY spot falls
below 50 after only ~37 units of market overhang (base 120, T 100, linear
decay at 1.60), which the opponent's own selling reaches routinely from the
mid game onward.

That gate treats a crashed spot as a reason to leave yield on the plant, but
HARVEST is not a sale: it moves yield from tile to shed, and rank_sales
already refuses to sell below hold_fraction * base.  Meanwhile unharvested
yield is destroyed by the engine two ways -- an at-cap ongoing crop silently
discards its next production, and an expired tile loses one unit every two
steps before turning to WEED -- while the town drains each product every 4
steps, so a crashed spot recovers.

v123b therefore prices a below-50 harvest at max(spot, hold_fraction * base)
instead of zero, but ONLY when the yield is genuinely at risk: the ongoing
crop is at its yield cap, or the tile expires within a day.  Yield that can
simply wait on the plant for the price to recover still waits, so the strong
days 0-19 phase keeps v122 behavior.  The floor is the agent's own selling reservation, so it is
never above what we would accept for the unit, and hold_fraction's existing
late-game decay retires the correction on its own.  Behavior is byte-identical
to v122 whenever spot >= 50.  No constant, threshold, crop score, animal
rule, assignment, sale, or telemetry definition changes.

v123b benchmarked 1071-529 (66.9%) against v122's 1058-542, but the counter it
targeted barely moved: STRAWBERRY expiry with held yield in days 20-24 went
776.02 -> 760.75 per game, 2%, against the 86-93% a single-seed probe had
predicted.  Instrumenting the job board explains why.  On a seed where the gate
fires on 92.5% of steps, the rescued HARVEST job is worth 9.5 and ranks 28th of
43 jobs -- unassignable with 12 hands.  It is worth 9.5 rather than
hold_fraction * 120 because two shed-load terms crush it, and both are active
exactly when the gate fires: hold_fraction clamps to 0.40 once shed_used passes
SHED_SOFT, and crop_service_jobs multiplies the harvest gain by 0.1 once
storage_load passes 90.  The shed averages 76/100 in days 20-24 and crosses 90
on 28% of steps.  So the shed, not the harvest price, is the binding resource.

What occupies it is not what v123b addressed.  rank_sales already dumps
STRAWBERRY whenever the rival holds any (reservation drops to 1.0), and it
does: sold on 100% of steps.  The residents are FERTILIZER, 19.8 units in days
20-24 rising to 36.5 in days 25-29, blocked from sale on 97.7% of steps, and
MILK at 13.1 units blocked on 100%.  We collect fertilizer at 8.57/day, apply
it at 2.72/day, and sell 0.28/day.

The block is adjusted_sell_hold.  Its reservation is a fraction of MP base,
which assumes a crashed price mean-reverts.  For a product we flood ourselves
it does not -- FERTILIZER spot sits at 29.8 then 17.9 against base 100, MILK at
16.0 then 18.8 against base 160.  Both relief branches that would clear a full
shed are conjoined with a cash test, and cash is never stressed: we lead the
panel by +16,785 through day 19.  So sell_hold stays pinned near 0.35 for the
whole late game and the shed never drains.

v125 is the sole behavioral change from v123b: the >= 80 relief branch fires on
storage pressure alone.  A shed at 80 of 100 is a binding constraint whether or
not the bank is healthy, and holding for a recovery that the opponent's supply
prevents costs the slot, the hold_fraction clamp, and the 0.1 harvest damping.
At sell_hold 0.08 the reservation clears FERTILIZER (8.0 against spot 29.8) and
MILK (12.8 against 16.0) while still refusing MELON (20.0 against 4.7), so the
genuinely worthless product is still held.  The >= 95 branch keeps its cash
test and no constant changes.

Phase confinement is by shed occupancy, not by a day constant, and that is not
the same thing.  Days 0-14 are untouched on both probed seeds -- shed load
means 29.9 and 37.7, never near 80.  Days 15-19 CAN be touched: on seed
267601732 the shed crosses 90 on 10% of steps in that window and v125's day
15-19 behavior does differ slightly from v123b's there.  The effect is small
(load 69.8 -> 70.2) and margin improved on that seed anyway, but this agent is
not phase-confined by construction and must not be described as such.

Reach is the reason to prefer this over more harvest-side work.  On seed
267601732 the STRAWBERRY spot never drops below 50 at all, so v122, v123b and
v124 produce byte-identical games; v125 still moves margin -19,691 -> -17,598.
The shed fills whether or not the strawberry market crashes.
"""
...
def adjusted_sell_hold(base_hold, cash, projected_used, n_beasts,
    ...
    # A shed at 80 of 100 is binding whether or not the bank is healthy.  The
    # reservation is a fraction of MP base, so it assumes a crashed spot
    # recovers; for a product we flood ourselves it does not, and the unsold
    # units then cost the slot, the hold_fraction clamp above SHED_SOFT, and
    # the storage_load > 90 damping on every crop harvest.
    if projected_used >= 80:
...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if (step < FINAL_FARM_STEP and cd["ong"]
        ...
        incremental = product_price - spot("FERTILIZER")
        strategic_denial = (tile.get("crop") in ("STRAWBERRY", "TOMATO")
                            and opponent_crop_counts.get(tile.get("crop"), 0) > 0)
        if due and (incremental > 12.0 or strategic_denial):
            fertilizer_jobs = 1
            jobs.append((max(80.0, 2.0 * product_price - spot("FERTILIZER")),
