...
def animal_allocation_limits(day, days_left, quadrant_count, slots,
                             beast_count, struct_count, pending_animals, crop_count):
    ...
    if days_left < 8:
        ...
        # Dynamic Opportunity Cost Gate: limit animals strictly to half of active crop count
        # This forces a "Crops First" expansion logic, starving the herd if crops fail
        allowed = max(4, int(crop_count * 0.50))
        target = min(16, max(current_assets, allowed))
...
def crop_service_jobs(x, y, tile, step, day, storage_load, spot,
    ...
    if held > 0 and age >= cd["fyd"]:
        ...
        # Value the harvest at the reservation price we would insist on when selling,
