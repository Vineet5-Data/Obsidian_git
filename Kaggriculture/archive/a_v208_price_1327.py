...
# v208: engine 1.32.7 added the "hinge" shape and moved CARROT / TOMATO / EGG
# onto it for the below-I0 (scarcity) side, with CARROT's below_target raised
# 0.20 -> 1.00.  Below the knee it is linear in x/T; past it a quadratic takes
# over, so a genuinely scarce product runs away in price.  f(T) == 1 by
# construction, so `target` keeps its meaning for every other shape.
#
# a_v186's table was copied from 1.32.6 and is now wrong on exactly the three
# belts the rival does not supply (tomato 0.00 units, egg 0.20, carrot 0.02).
# Measured against the live engine, the stale model under-prices TOMATO at 600
# units short by $768 (thinks 132, engine pays 900), EGG by $104, CARROT by $71
# -- so the agent can neither chase nor defend a scarcity spike it cannot see.
HINGE_GAIN = 8.0
