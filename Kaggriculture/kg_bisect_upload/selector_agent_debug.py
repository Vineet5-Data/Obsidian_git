...
def agent(obs, config=None):
    try:
        ...
        import traceback
        with open('agent_crash.log', 'w') as err_f:
...
def _plan(obs):
  ...
  },
  {
    ...
    # RESTORE EARLY GAME LOGIC FROM v97_cap70
    if days_left < 8:
    ...
    for item, raw_qty in projected.items():
        ...
        if sell_hold <= 0.0:
            ...
            shop_demand = {"WHEAT": 0, "STRAWBERRY": 0, "MILK": 0, "EGG": 0, "CARROT": 0, "TOMATO": 0, "WOOL": 0, "MELON": 0}
            for s in shops:
                if s == "YARN_STORE": shop_demand["WOOL"] += 2
                elif s == "BAKERY": shop_demand["WHEAT"] += 1; shop_demand["EGG"] += 1; shop_demand["MILK"] += 1
                elif s == "GREENGROCER": shop_demand["CARROT"] += 1; shop_demand["TOMATO"] += 1; shop_demand["STRAWBERRY"] += 1
                elif s == "DAIRY": shop_demand["MILK"] += 2
                elif s == "FARMERS_MARKET": shop_demand["WHEAT"] += 1; shop_demand["STRAWBERRY"] += 1
            drain_ratio = shop_demand.get(item, 0) / max(1, qty)
            scaled_hold = sell_hold * max(1.0, drain_ratio * 1.5)
            reservation = max(1.0, scaled_hold * MP[item]["base"])
    ...
    impact_weights = {"MELON": 3.5, "STRAWBERRY": 2.0, "MILK": 2.0, "WOOL": 3.2}
    rows.sort(key=lambda r: (impact_weights.get(r[2], 1.0) * r[3], r[0]), reverse=True)
