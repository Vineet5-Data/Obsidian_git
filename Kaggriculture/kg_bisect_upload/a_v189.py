...
def animal_allocation_limits(day, days_left, quadrant_count, slots,
    ...
    if days_left < 8:
        ...
        target = min(14, max(4, int(round(25 * quadrant_count * 0.18))))
    need = max(0, target - beast_count - pending_animals)
    structure_need = min(slots, max(0, target - current_assets))
    species_cap = max(2, int(math.ceil(max(1, target) * 0.75)))
...
def _plan(obs):
    ...
    if day >= 20:
        crop_cap_share = 0.55 if len(shops) < 2 else 0.65
    elif day >= 5:
        crop_cap_share = 0.60 if len(shops) < 2 else 0.65
    else:
    ...
    for crop in ("STRAWBERRY", "MELON", "TOMATO"):
        if crop == "TOMATO" and absorb.get("TOMATO", 0) <= 0.5:
            continue
