...
def quad_of(x, y):
    """Mirrors the engine's _quadrant_of (kaggriculture.py:114)."""
    h = BOARD // 2
...
def _plan(obs):
    ...
    # Crop blocks, not a crop soup.
    #
    # `empties` is ordered by distance to the shed, and the shed sits at the
    # board centre -- so a distance ring spans all four quadrants at once.
    # Offering every crop the same `crop_empties[:n]` prefix therefore let crops
    # interleave tile-by-tile across the whole farm: neighbouring tiles ripened
    # on different days, so a unit could never plant/water/harvest two tiles in
    # a row without walking.  That is the 1.6 moves-per-productive-action tax
    # measured in the Kaggle loss replays (opponents run ~1.0).
    #
    # Fix is two-part and purely a function of the observation:
    #   1. group tiles by quadrant, quadrants ordered by proximity to the shed,
    #      so a crop's tiles stay inside one 5x5 block;
    #   2. hand each crop a *disjoint* slice via a cursor (same idiom the animal
    #      siting above already uses), so blocks never overlap.
    # A block then ripens together and a unit works it in place.
    quad_rank = {}
    for p in crop_empties:
        q = quad_of(p[0], p[1])
        d = min(dist(p, s) for s in shed_access_set)
        if q not in quad_rank or d < quad_rank[q]:
            quad_rank[q] = d
    crop_empties.sort(key=lambda p: (quad_rank[quad_of(p[0], p[1])],
                                     quad_of(p[0], p[1]),
                                     min(dist(p, s) for s in shed_access_set),
                                     p[1], p[0]))
    ...
    for crop, n_want in sorted(want_crop.items(), key=lambda kv: -kv[1]):
        ...
        # +4 overhang keeps candidates if some tiles in the block fall through;
        # the cursor still advances by n_want so blocks stay essentially disjoint.
        block = crop_empties[crop_cursor:crop_cursor + n_want + 4]
        crop_cursor += n_want
        for x, yy in block:
